...
| 4.111.2 | 2026-08-26 | 新增 Google Play 商店與官網行銷截圖素材：`docs/screenshots/{app,web}/{light,dark}/{locale}` 存放真實 UI 截圖（含登入頁 Google SSO 選項），`docs/android-light`／`docs/android-dark`／`docs/web-marketing` 存放帶標題文案的正式行銷截圖（透過 app-store-screenshots skill 匯出）；`docs/marketing-screenshots/{app-dark,app-light,web}` 為對應的可編輯截圖專案（Next.js，含 `app-store-screenshots.json` 版面設定）。`README.md`「螢幕截圖」章節改用上述真實截圖，移除從未實際產生內容的 `marketing-screenshots/{zh,en,ar}`、`web-marketing-screenshots/{zh,en,ar}` 失效連結與過時的登入流程截圖，並重寫專案優勢／目錄結構。`.gitignore` 新增 `docs/android-light`／`docs/android-dark`／`docs/web-marketing` 允許規則、`docs/marketing-screenshots/*/*.log` 忽略規則；`mobile/.gitignore` 新增 `/web/`，排除擷取截圖時暫時建立、不隨產品出貨的 Flutter Web 平台檔案。另外，`lib/db.ts` 新增 `alterIgnore` 補建 `stocks.shares`／`stock_transactions.shares`／`stock_dividends.shares`（`NUMERIC DEFAULT 0`），修正部分舊版 PostgreSQL 部署因欄位缺失導致 4.111.0 數值型別遷移失敗的問題；`app/globals.css` 修正 `--dashboard-ink`／`--dashboard-ink-soft` 淺色模式配色誤用暗色系數值，導致儀表板頂部卡片與訊號列文字在亮色模式下難以閱讀的問題。驗證：README 圖片路徑全數對應檔案系統中存在的檔案，`git status` 確認無殘留建置產物或未過濾的暫存記錄檔。 |
...
| 4.105.0 | 2026-08-22 | Dashboard 新增財務訊號列，集中顯示待處理提醒、現金展望期末餘額與投資組合未實現損益；提醒與兩項財務訊號皆可直接前往對應頁面，手機版改為直向排列並保留 RTL、鍵盤焦點與 reduced-motion 支援。使用既有 Dashboard 資料與翻譯，不變更 API contract、計算邏輯、schema 或 dependency。驗證：`npm run typecheck`、Dashboard targeted tests、`npm run build`、`git diff --check` 通過。 |
| 4.103.0 | 2026-08-16 | MCP 新增三個信用卡還款工具（見 `specs/007-mcp-credit-card-repayment/`）。`lib/creditCardRepayment.ts` 新增三個共用函式 `computeRepaymentAllocation()`／`executeRepayment()`／`evaluateRepaymentSummary()`，從既有兩支 HTTP 路由（`POST /api/accounts/credit-card-repayment`、`GET /api/credit-card-repayment-summaries/{id}`）原樣抽出，供既有路由與三個新 MCP 工具共用同一份計算與寫入邏輯，既有路由行為不變。`lib/mcpServer.ts` 的 `buildMcpServer()` 新增三個工具：`get_credit_card_repayment_preview`（唯讀試算，無條件註冊、不受 `allowCreate` 影響，驗證規則與錯誤訊息逐字重用寫入工具，但不呼叫 `executeRepayment()`，在協定層面不觸及任何寫入路徑）、`list_credit_card_repayments`（唯讀查詢，分頁＋日期篩選，對每一列呼叫 `evaluateRepaymentSummary()` 標示各卡 `intact`／`modified`／`deleted` 與整體 `stale`）、`create_credit_card_repayment`（以一個總金額送出還款，僅在 `credential.allowCreate === true` 時才 `registerTool()`，與 `create_transaction` 綁同一旗標同進同出；驗證規則 V1–V5 逐字重用既有 `POST` 路由，以送出當下重新 `collectPayableCards()` 計算，呼叫 `computeRepaymentAllocation()` + `executeRepayment()` 寫入並標記 `aiCreated: true`，支援 `idempotencyKey` 冪等鍵沿用既有 `mcp_transaction_idempotency` 表 24 小時 TTL，成功後以 `withAudit()` 寫入 `action: 'mcp_create_credit_card_repayment'`、`metadata.repayment_summary_id`）。`lib/auditHelpers.ts` 的 `AUDIT_METADATA_ALLOWED_KEYS` 新增 `repayment_summary_id`（刻意不重用 `transaction_id`，避免稽核閱讀者誤判查詢對象）；`components/features/admin/AdminClient.tsx` 的 `AUDIT_ACTION_LABELS` 新增 `mcp_create_credit_card_repayment` 中文標籤「MCP 信用卡還款」。新增 `tests/lib/mcpCreditCardRepayment.test.ts`（需 `DATABASE_URL`，涵蓋三個工具端到端、V1–V5 拒絕、`ai_created` 標記、冪等鍵、稽核），擴充 `tests/lib/mcpDescriptors.test.ts` 驗證 `allowCreate=true` 憑證 `tools/list` 額外出現 `create_credit_card_repayment` 且 annotations 標示可寫非破壞性、兩個唯讀工具在未開 `allowCreate` 時仍可見，並擴充 `tests/lib/creditCardRepayment.test.ts` 涵蓋三個共用函式；`package.json` 新增 `test:mcp-credit-card-repayment` 腳本並納入 `npm test` 主鏈。`asset_openapi.yaml` 無新增 HTTP 端點（MCP 工具非 REST 路徑），僅將 `info.version` 由 `4.102.0` 更新至 `4.103.0`。驗證：`npm run test:mcp-descriptors`、`npm run test:credit-card-repayment` 通過（DB 整合案例因本機無 `DATABASE_URL` 略過）。 |
| 4.102.0 | 2026-08-16 | 信用卡總金額還款：輸入一個總金額，系統依各卡欠款等比例自動分配到付款帳戶關聯的所有信用卡（見 `specs/006-credit-card-total-repayment/`）。`lib/db.ts` 的 `_runMigrations()` 新增冪等 `ALTER transactions ADD COLUMN repayment_summary_id TEXT DEFAULT ''`（非還款交易恆為空字串），並新增資料表 `credit_card_repayment_summaries`（`id`／`user_id`／`date`／`from_account_id`／`from_account_name`／`from_currency`／`total_amount`／`input_mode`／`allocations`（JSON 快照）／`created_at`／`updated_at`）與兩支索引（`idx_ccr_summaries_user`、`idx_transactions_repayment_summary` 帶 `WHERE repayment_summary_id != ''` 的部分索引）；無任何資料回填或歷史重算，部署啟動自動套用。新增共用模組 `lib/creditCardRepayment.ts`（`collectPayableCards(userId, fromAccountId, fromCurrency)`：依納入規則 `account_type='信用卡' AND linked_bank_id=fromAccountId AND 餘額<0`，欠款換算同幣別 `round_half_up(-balance)`、異幣別經兩個匯率換算後 half-up、換算後 `<1` 設為 1，依 `created_at` 早→晚、相同再 `id` 升冪排序）與 `lib/creditCardRepaymentAllocation.ts`（`allocateRepayment(totalAmount, cards)` 純函式、純整數 BigInt 運算、無 DB／無 Next.js 相依；七步分配演算法含前置／後置條件 assert，殘差併入欠款最大卡、保底每卡 ≥1、超額依金額大→小扣減；因 tsconfig target ES2017 禁 BigInt 字面量，常數以 `BigInt(n)` 建構）。新增端點 `POST /api/accounts/credit-card-repayment`：以送出當下重新取得的欠款計算（不沿用前端快 snapshot），同一交易內寫入 2N 筆轉帳交易 ＋ 1 筆摘要，任一例外一律 `ROLLBACK` 回 500；新增端點 `GET /api/accounts/{id}/repayment-cards`（付款帳戶快照，每卡只輸出 5 欄、不輸出 `created_at`，付款帳戶為信用卡回 400 `NotCreditCardPayer`）、`GET /api/credit-card-repayment-summaries/{id}`（事後讀取分配摘要，逐卡比對當下交易狀態標示 `intact`／`modified`／`deleted`，金額比對容差 0.005，`fromAccount.name`／`.currency` 取自摘要快照欄位不即時 JOIN）。前端新增 `components/features/accounts/CreditCardRepaymentDialog.tsx`（付款帳戶選擇、總金額輸入、本地 `allocateRepayment` 重算預覽不往返、送出後渲染實際分配結果含預繳標記），`AccountsClient.tsx`／`TransactionsClient.tsx` 整合還款入口與「檢視還款分配」。行動 App 對應實作：`mobile/lib/credit_card_repayment_allocation.dart`（Dart 對照實作，逐步驟對應 TS，以同一份 `shared/repayment-allocation/cases.json` 黃金測資釘住）；`mobile/lib/api_client.dart`／`models.dart`／`screens/accounts_screen.dart`／`screens/transactions_screen.dart` 新增還款寫入、快照、分配摘要檢視與預繳標記。`lib/i18n/dictionaries/*.ts`（10 語言）／`shared/i18n/app_*.arb`／行動端 ARB 與產生檔新增 `repayment.totalAmount`／`totalAmountHint`／`totalDebt`／`allocationPreviewTitle`／`colCard`／`colAllocated`／`colBalanceAfter`／`prepaidBadge`／`prepaidAmount`／`resultTitle`／`resultDone`／`messages.repaymentTotalAmountInvalid`／`messages.repaymentTotalAmountTooSmall`／`transactions.viewRepaymentAllocation`／`repaymentSummaryTitle`／`repaymentSummaryTotal`／`repaymentSummaryStale`／`repaymentSummaryStatusIntact`／`repaymentSummaryStatusModified`／`repaymentSummaryStatusDeleted` 等鍵，經 `npm run i18n:generate` 同步（含 `flutter gen-l10n`），i18n 鍵數由 1,334 增至 1,354。`asset_openapi.yaml` 新增三組還款路徑契約，`info.version` 由 `4.101.0` 更新至 `4.102.0`。新增 `tests/lib/creditCardRepayment.test.ts`／`tests/lib/creditCardRepaymentAllocation.test.ts`（純函式含黃金測資與隨機案例）、`mobile/test/credit_card_repayment_allocation_test.dart`（Dart 端對照），並擴充 `tests/lib/db.test.ts` 涵蓋新資料表與欄位；`package.json` 新增 `test:repayment-allocation`／`test:credit-card-repayment` 兩支腳本並納入 `npm test` 主鏈。驗證：`npm run typecheck`、`npm test`（21 個檢查項目：19 支 `test:*` 腳本＋`check:iso`＋`check:i18n` 10 語言 1,354 鍵對齊）皆通過；`test:credit-card-repayment` 之 DB 整合案例因本機無 `DATABASE_URL` 略過。 |
| 4.101.0 | 2026-08-15 | 交易記錄顯示 AI／MCP 修改標記並支援使用者還原（見 `specs/005-mcp-transaction-restore/`）。`lib/db.ts` 的 `_runMigrations()` 新增三個冪等 `ALTER`：`transactions.ai_created`／`note_ai_modified` INTEGER NOT NULL DEFAULT 0、`pre_ai_note` TEXT NOT NULL DEFAULT ''；並補兩段一次性回填 `UPDATE`：第一段依 `data_operation_audit_log` 中 `action = 'mcp_create_transaction'` 的 `metadata->>'transaction_id'` 回填既有交易的 `ai_created = 1`，第二段沿 `linked_id` 傳播一跳（涵蓋轉帳另一腳與自動產生的手續費子交易）；兩段皆為冪等 `UPDATE`，重跑不影響已是 1 的列。`lib/transactionWriteCore.ts` 的 `insertIncomeExpenseTransaction()`／`insertTransferPair()` 新增 `aiCreated?` 輸入並寫入新欄位；新增匯出函式 `deleteTransactionCascade(userId, txId, linkedId)`，從原本內嵌在 `DELETE` handler 的級聯刪除邏輯抽出（含 `deleteTransactionAttachments()` 與 `linked_id` 配對列刪除），供既有 `DELETE app/api/transactions/[txId]/route.ts` 與新的還原端點共用，回傳實際被移除的 id 陣列；`lib/overseasFee.ts` 的 `insertFeeTransaction()` 同步新增 `aiCreated?`，讓 AI 建立轉帳觸發的自動海外手續費子交易一併標記。`lib/mcpServer.ts`：`create_transaction` 工具（一般收支與轉帳兩路徑）呼叫上述函式時固定帶入 `aiCreated: true`；`update_transaction_note` 工具原本「單一 `UPDATE` 只寫 `note`／`updated_at`」的陳述式擴充為同一陳述式內以 `pre_ai_note = note`（取異動前舊值）與 `note_ai_modified = 1` 一併寫入，維持不做讀改寫的既有保證。`app/api/transactions/[txId]/route.ts` 的 `PUT`／`PATCH` 共用 `updateHandler()`：因既有前端整份表單協定恆送出 `note`，新增「新舊 `note` 值是否真的不同」的純記憶體字串比對（`existing` 已由既有 `SELECT *` 取得，不新增 DB 往返）——不同則連同 `note` 一併清除 `note_ai_modified = 0`／`pre_ai_note = ''`，相同（同值重送）則原樣保留；`ai_created` 欄位不出現在這次 `UPDATE` 陳述式中，不受任何手動編輯影響（FR-002）。`DELETE` handler 改為呼叫 `deleteTransactionCascade()`，行為不變。`app/api/transactions/route.ts`（`GET` 列表）的 `TransactionListItem` 新增 `aiCreated`／`noteAiModified` 兩個布林欄位。新增端點 `app/api/transactions/[txId]/restore-ai-created/route.ts`（`POST`）：`ownsResource()` 確認歸屬後，非 `ai_created = 1` 回 409 `NotRestorable`；否則呼叫 `deleteTransactionCascade()`，成功後 `auditSensitiveAction()` 寫入 `action: 'restore_ai_created_transaction'`，`metadata` 僅含 `transaction_id`／`linked_transaction_id`（新增進 `lib/auditHelpers.ts` 的 `AUDIT_METADATA_ALLOWED_KEYS` 白名單）。新增端點 `app/api/transactions/[txId]/restore-ai-note/route.ts`：`GET` 為純讀預覽（US3），回傳 `restorable`／`preAiNote`（僅 `restorable` 時給值，否則 `null`）／`currentNote`／`updatedAt`，不變更任何資料；`POST` 帶 `body.expectedUpdatedAt`，先呼叫既有 `assertOptimisticLock()` 重新確認當下狀態（FR-017），衝突時攔截為專屬訊息與 `code: 'NoteChanged'`（而非泛用鎖定錯誤），通過後再檢查 `note_ai_modified === 1`（否則 409 `NotRestorable`），單一 `UPDATE` 陳述式將 `note` 設回 `pre_ai_note`、`note_ai_modified = 0`、`pre_ai_note = ''`，成功後稽核 `action: 'restore_ai_modified_note'`，`metadata` 僅含 `transaction_id`（不含備註全文，比照既有 FR-012 保守揭露原則）。`components/features/admin/AdminClient.tsx` 的 `AUDIT_ACTION_LABELS` 新增 `restore_ai_created_transaction`／`restore_ai_modified_note` 中文標籤，未新增任何備註內容相關的 metadata 標籤。前端 `components/features/transactions/TransactionsClient.tsx`：卡片與表格兩種既有版面在 `note`／`isFxFee` 等既有條件旁新增 `tx.aiCreated`／`tx.noteAiModified` 兩個獨立徽章；操作列依條件新增「還原」（`Undo2`）與「還原備註」（`RotateCcw`）按鈕；新增兩個確認 Modal——還原建立為既有刪除類 danger 按鈕直接確認，還原備註的 Modal 先呼叫 `GET .../restore-ai-note` 取得預覽（US3）並排顯示 `currentNote`／`preAiNote`，確認後才 `POST` 並帶入預覽取得的 `updatedAt` 作為樂觀鎖依據。行動 App 對應實作：`mobile/lib/models.dart` 的 `Txn` 新增 `aiCreated`／`noteAiModified`／`updatedAt` 三個欄位（`fromJson` 相容 camelCase 與既有 snake_case 回應）；`mobile/lib/api_client.dart` 新增 `restoreAiCreatedTransaction()`／`restoreAiNote()`／`getAiNoteSnapshot()`；`mobile/lib/screens/transactions_screen.dart` 的 `_TransactionsScreenState` 新增 `_restoreAiCreated()`／`_restoreAiNote()` 確認對話框處理函式，透過 `_TxnTile` 新增的 `onRestoreCreated`／`onRestoreNote` 回呼接上列表項目，呈現與網頁版一致的徽章與還原按鈕（`Icons.restore`／`Icons.undo`）。`lib/i18n/dictionaries/*.ts`（10 語言）／`shared/i18n/app_*.arb`／行動端對應 ARB 與產生檔新增 `aiCreated`、`noteAiModified`、`restoreCreated`、`restoreCreatedTitle`、`restoreCreatedMessage`、`restoreNote`、`restoreNoteTitle`、`restoreNoteConfirmMessage`、`restoreNoteCurrentLabel`、`restoreNotePreviewLabel` 共 10 鍵，經 `npm run i18n:generate` 同步（含 `flutter gen-l10n`）。`asset_openapi.yaml` 新增 `/api/transactions/{id}/restore-ai-created`（`POST`）與 `/api/transactions/{id}/restore-ai-note`（`GET`／`POST`）兩組路徑契約，`info.version` 由先前落後未跟上 4.100.0 的 `4.99.0` 追上 `4.101.0`。新增 `tests/lib/db.test.ts`（驗證兩段回填 `UPDATE` 陳述式的正確性與冪等性；因 `initDB()` 同行程內建「已初始化即直接 return」守門，無法真正重跑 migration，改為直接對 fixture 執行相同 SQL）、`tests/lib/transactionRestore.test.ts`（涵蓋兩個新端點成功／失敗路徑、轉帳整組還原、404／409 各類錯誤碼、稽核寫入與不含備註全文）、`tests/lib/transactionsManualEdit.test.ts`（本專案首次以已認證 Route Handler 方式測試，驗證手動編輯「備註值真的改變」才清除標記、同值重送保留標記、`ai_created` 標記不受手動編輯影響），並擴充既有 `tests/lib/mcpCreateTransaction.test.ts`／`tests/lib/mcpUpdateTransactionNote.test.ts`／`tests/lib/transactionWriteCore.test.ts` 涵蓋 `aiCreated`／`note_ai_modified` 相關斷言；`package.json` 新增 `test:db-migrations`／`test:transactions-manual-edit`／`test:transaction-restore` 三支腳本並納入 `npm test` 主鏈。驗證：`npm run typecheck`、`npm test`（20 個檢查項目：18 支 `test:*` 腳本＋`check:iso`＋`check:i18n` 10 語言 1,334 鍵對齊）皆通過；上述三個新測試檔與 `mcpCreateTransaction`／`mcpUpdateTransactionNote`／`transactionWriteCore` 之 DB 整合案例因本機無 `DATABASE_URL` 略過。 |
| 4.100.0 | 2026-08-15 | MCP 新增 `update_transaction_note` 工具，僅能更新既有交易的備註欄位，其餘欄位一律不可變更、無刪除能力（見 `specs/004-mcp-update-notes-only/`）。抽出共用模組 `lib/transactionEditRules.ts`：`findTransactionEditBlock()`（`is_fx_fee === 1` 之「列本身不可編輯」規則，供 `app/api/transactions/[txId]/route.ts` 的 `PATCH` 與新工具共用，錯誤訊息與狀態碼逐字相同）、`TRANSACTION_NOTE_MAX_LENGTH = 200`（供 `components/features/transactions/TransactionsClient.tsx` 手動編輯表單 `maxLength` 與新工具的伺服器端驗證共用同一常數，取代先前兩處硬編碼 200）；此模組刻意不 import `lib/db.ts`，避免伺服器端相依污染前端 bundle。`lib/mcpServer.ts` 的 `buildMcpServer()` 新增 `update_transaction_note` 工具，僅在 `credential.allowUpdateNote === true` 時才 `registerTool()`（未開啟則 `tools/list` 不出現該工具），與 `create_transaction` 的 `credential.allowCreate` 判斷互相獨立、不巢狀；輸入以 `zod` schema 僅接受 `transactionId`／`note`（非 `.strict()`，夾帶其他欄位靜默剝除不報錯）；處理順序為長度驗證（>200 拒絕）→ 依 `id`+`user_id` 查列 → `findTransactionEditBlock()` 擋不可編輯交易 → 單一 `UPDATE ... SET note = ?, updated_at = ?` 陳述式（不做讀改寫）→ 重新查列組回應（欄位形狀與 `list_transactions` 單筆項目一致）→ 成功時才寫入稽核。新增 `lib/mcpOpenAiCompatibility.ts` 的 `createUpdateOAuthToolDescriptor()`，`annotations` 採 `destructiveHint: true`／`idempotentHint: true`（與 `create_transaction` 的 `WRITE_TOOL_ANNOTATIONS` 刻意不同：覆寫既有備註值不符合「僅做加法」定義）。新增資料表欄位 `mcp_credentials.allow_update_note`／`mcp_oauth_connections.allow_update_note`（`lib/db.ts` 冪等 `ALTER`，預設 0，與 `allow_create` 各自獨立，不因既有 `allow_create = 1` 而預設開啟；關閉權限不回溯影響已更新的備註）；`lib/mcpAuth.ts`／`lib/mcpOAuth.ts` 的 `verifyMcpToken()`／`verifyMcpOAuthAccessToken()`／`listMcpCredentials()`／`listMcpOAuthConnections()`／`serializeCredential()` 一併回傳 `allowUpdateNote`，新增 `setMcpCredentialAllowUpdateNote()`／`setMcpOAuthConnectionAllowUpdateNote()`（OAuth 連線比照既有 `allow_create` 行為：授權仍有效時重新授權不重置，全數撤銷後重新授權則重置為 0）。`PATCH app/api/user/mcp-credentials/[id]/route.ts`／`PATCH app/api/user/mcp-oauth-connections/[clientId]/route.ts` 改為同時接受 `allowCreate`／`allowUpdateNote`（body key 存在即視為要更新該欄位，兩者皆選填但至少擇一，皆缺回傳 400；未提供的欄位維持原狀不動）。前端 `components/features/settings/McpSettingsClient.tsx`／`McpConnectionsClient.tsx` 新增「允許更新備註」欄位與 checkbox；`components/features/admin/AdminClient.tsx` 的 `AUDIT_ACTION_LABELS` 新增 `mcp_update_transaction_note` 中文標籤。稽核 metadata 僅含 `transaction_id`，刻意不含 `transaction_summary`（本功能不變更金額）與備註全文；被拒絕的嘗試（長度超限、交易不存在、不可編輯、非本人）一律不寫入稽核。`asset_openapi.yaml`：`McpCredentialAllowCreateUpdate`／`McpOAuthConnectionAllowCreateUpdate` 更名為 `McpCredentialPermissionUpdate`／`McpOAuthConnectionPermissionUpdate`（屬性與端點路徑不變，`operationId` 改為 `setMcpCredentialPermissions`／`setMcpOAuthConnectionPermissions`），`McpCredential`／`McpOAuthConnection` 新增必填 `allowUpdateNote`，`info.version` 更新為 `4.100.0`。新增 `tests/lib/transactionEditRules.test.ts`（純函式、無需資料庫）、`tests/lib/mcpUpdateTransactionNote.test.ts`（需 `DATABASE_URL`，涵蓋成功更新、逐欄不變比對、回應形狀一致、夾帶欄位靜默忽略、不可編輯交易被拒、非本人交易被拒、刪除工具不存在、空字串清空、長度上限、轉帳單腳獨立、冪等、久遠交易、缺欄位被拒、稽核形狀與不含備註全文、被拒絕不留稽核），並擴充 `tests/lib/mcpAuth.test.ts`／`tests/lib/mcpOAuth.test.ts`／`tests/lib/mcpDescriptors.test.ts` 涵蓋 `allowUpdateNote` 預設值、開關獨立性、`tools/list` 可見性與 OAuth 重新授權重置情境。驗證：`npm run typecheck`、`npm test`（17 個檢查項目：15 支 `test:*` 腳本＋`check:iso`＋`check:i18n` 10 語言鍵值對齊）皆通過；`test:mcp-update-note` 之 DB 整合案例因本機無 `DATABASE_URL` 略過。 |
| 4.99.0 | 2026-08-14 | MCP AI 新增交易紀錄，僅限建立、任何情況下皆無刪除／修改既有資料的工具（見 `specs/003-mcp-write-no-delete/`）。`lib/mcpServer.ts` 的 `buildMcpServer()` 新增 `create_transaction` 工具，僅在 `credential.allowCreate === true` 時才 `registerTool()`（未開啟則 `tools/list` 完全不出現該工具，刪除／修改工具則不存在於任何程式碼路徑，不因權限而有例外）；輸入以 `zod` schema 驗證 `type`（income/expense/transfer）、`amount`（>0）、`currency`／`fxRate`／`date`／`categoryId`／`accountId`／`fromAccountId`／`toAccountId`／`note`／`idempotencyKey`。新增 `lib/transactionWriteCore.ts` 的 `insertIncomeExpenseTransaction()`／`insertTransferPair()`，抽取自既有手動建立交易的計算邏輯（金額精度、幣別換算 `convertToTwd()`、跨幣別海外手續費 `resolveOverseasFee()`、轉帳雙腳寫入），`app/api/transactions/route.ts`（`POST`）與 `app/api/transactions/transfer/route.ts` 改為呼叫抽取後的共用函式，行為不變；MCP 新增交易與手動建立共用同一份計算邏輯，帳戶／分類受理政策維持兩者既有各自差異（MCP 缺帳戶或有多帳戶未指定則拒絕，分類不存在則歸未分類，比照既有 CSV 匯入路徑）。新增資料表欄位 `mcp_credentials.allow_create`（`lib/db.ts` 冪等 ALTER，預設 0）供 PAT 憑證逐憑證開關；新增資料表 `mcp_oauth_connections`（`user_id`+`client_id` 複合主鍵、`allow_create`、`first_connected_at`、`last_used_at`）代表「已連接 AI 工具」，獨立於既有 `mcp_oauth_tokens` 的 token／scope 輪替，`lib/mcpOAuth.ts` 的 `exchangeMcpAuthorizationCode()` 於核發 token 時以 `ON CONFLICT` upsert：既有有效 token 時僅更新 `last_used_at`／`client_name`，全數失效後重新授權則一併把 `allow_create` 重置為 0；新增 `listMcpOAuthConnections()`（僅回傳目前仍有未撤銷未過期 token 的連線）／`setMcpOAuthConnectionAllowCreate()`。新增資料表 `mcp_transaction_idempotency`（`credential_id`+`idempotency_key` 唯一索引、`expires_at`），`create_transaction` 呼叫時先查快取命中則直接回傳先前結果，否則建立後以 `ON CONFLICT DO NOTHING` 寫入（24 小時 TTL），處理併發下兩個相同 key 請求同時建立的極端情況；冪等鍵命中快取不視為新增操作，不重複寫入稽核。新增 API：`PATCH app/api/user/mcp-credentials/[id]/route.ts`（切換 PAT 的 `allowCreate`）、`app/api/user/mcp-oauth-connections/route.ts`（`GET` 列表）、`app/api/user/mcp-oauth-connections/[clientId]/route.ts`（`PATCH` 切換）；`serializeCredential()` 由 `app/api/user/mcp-credentials/route.ts` 改移至 `lib/mcpAuth.ts` 供 GET／PATCH 共用（Next.js route.ts 只能匯出識別的 HTTP handler，原本額外具名匯出供同目錄 `[id]/route.ts` 匯入會被 typed routes 檢查判定為不合法的 route export，`npm run typecheck` 會失敗）。新增前端頁面 `app/settings/mcp-connections/page.tsx` + `components/features/settings/McpConnectionsClient.tsx`（「已連接 AI 工具」清單與開關），`components/features/settings/McpSettingsClient.tsx` 既有 PAT 清單新增「允許新增資料」欄位；`components/layout/Sidebar.tsx` 新增導覽項目。`lib/mcpServer.ts` 的 `withAudit()` 擴充 `action`／`extraMetadata` 參數，`create_transaction` 成功後寫入 `action: 'mcp_create_transaction'`、`metadata.transaction_summary`（僅金額與收支類型，不含備註全文）；`components/features/admin/AdminClient.tsx` 的 `AUDIT_ACTION_LABELS`／`AUDIT_META_LABELS` 新增對應中文標籤（含補上既有 `mcp_query` 遺留的英文標籤落差）。速率限制沿用 `proxy.ts` 既有 `/api/mcp` 全域限流，未新增獨立機制。新增 `tests/lib/transactionWriteCore.test.ts`／`tests/lib/mcpCreateTransaction.test.ts`（DB 依賴部分於無 `DATABASE_URL` 時略過），`tests/lib/mcpDescriptors.test.ts` 新增案例驗證 `allowCreate=true` 憑證的 `tools/list` 額外出現 `create_transaction` 且 annotations 標示可寫非破壞性，並驗證刪除／修改語意的不存在工具名稱一律回傳工具不存在錯誤、既有交易欄位不受影響。`asset_openapi.yaml` 補上本次新增端點的契約並將 `info.version` 由既有落後的 `4.12.0` 追上 `4.98.2`（僅為文件契約，未被任何 CI 流程消費，不影響建置或測試）。驗證：`npm run typecheck`、`npm test`（15 支測試腳本＋`check:iso`＋`check:i18n` 10 語言 1,318 鍵對齊）皆通過；`test:mcp-create-transaction`／`test:transaction-write-core` 之 DB 整合案例因本機無 `DATABASE_URL` 略過；quickstart.md 七章節端對端手動驗收與 `asset_openapi.yaml` 升版後的 Cloudflare API Shield 契約相容性查核未執行（前者需真實 AI 工具連線環境，後者依 spec 標註不阻擋合併）。 |
| 4.98.2 | 2026-08-13 | MCP OAuth 新增 `private_key_jwt`（RS256／JWKS）client authentication，修正 ChatGPT connector 的 CIMD metadata（`token_endpoint_auth_method: private_key_jwt`、`jwks_uri`）在 4.98.1 僅支援 `none`／`client_secret_basic`／`client_secret_post` 之外仍會授權失敗的獨立成因。`lib/mcpOAuthCore.ts` 的 `normalizeMcpOAuthClientMetadata()` 新增 `allowPrivateKeyJwt` 選項（`registerMcpOAuthClient()`／`validateClientIdMetadataDocument()` 開啟，DCR 沿用既有 secret 方式），驗證 `jwks_uri` 必須為 HTTPS、`token_endpoint_auth_signing_alg` 僅接受 `RS256`；新增欄位 `mcp_oauth_clients.jwks_uri`／`token_endpoint_auth_signing_alg`（`lib/db.ts` 冪等 ALTER）。`lib/mcpOAuth.ts` 新增 `verifyMcpOAuthClientAssertion()`：解析 JWT 三段式結構、以 client 提供的 JWKS 找出對應 `kid` 的 RSA 公鑰（僅接受 `kty=RSA`、非 `enc` 用途、最小 2048-bit）驗證 `RS256` 簽章，檢查 `iss`/`sub` 等於 `client_id`、`aud` 等於 token／revocation endpoint、`exp`／`iat`／`nbf` 於允許誤差內、`jti` 格式合法；新增資料表 `mcp_oauth_client_assertions`（`client_id`+`jti` 複合主鍵）供一次性 `jti` 重放保護，`getClientJwks()` 以 SSRF 防護（HTTPS-only、僅接受公開 IP、無重導向、大小與逾時限制）擷取並快取 JWKS（`kid` 未命中時強制重新整理一次再判定失敗）。`parseMcpOAuthClientCredentials()`／`verifyMcpOAuthClientAuthentication()` 新增 `client_assertion_type`／`client_assertion` 參數解析與非同步驗證（需傳入 audience，`app/api/oauth/token`／`app/api/oauth/revoke` 對應改為 `await`）；`createMcpAuthorizationServerMetadata()` 的 `token_endpoint_auth_methods_supported`／`revocation_endpoint_auth_methods_supported` 新增 `private_key_jwt`；`app/mcp/page.tsx` 補充 CIMD 支援 `private_key_jwt` 的說明。新增/擴充 `tests/lib/mcpOAuth.test.ts` 涵蓋以實際 RSA 金鑰產生的斷言簽章驗證、`jti` 重放拒絕、`kid` 找不到時的 JWKS 強制刷新。驗證：`npm run test:mcp-oauth` 15/15、`npm run typecheck`、`npm test`、`npm run build`、`npm audit`（0 vulnerabilities）、`git diff --check` 均通過。 |
| 4.98.1 | 2026-08-13 | MCP OAuth DCR（`lib/mcpOAuthCore.ts` normalizeMcpOAuthClientMetadata／`lib/mcpOAuth.ts`）新增支援 `client_secret_basic`／`client_secret_post` confidential client（除既有 `none` public client 外），註冊時簽發高熵 client secret 並以 SHA-256 hash 存於新增欄位 `mcp_oauth_clients.client_secret_hash`／`client_secret_expires_at`（`lib/db.ts` 冪等 ALTER），secret 僅於註冊回應回傳一次、`serializeRegisteredClient()` 不外洩 hash；`app/api/oauth/token`／`app/api/oauth/revoke` 新增 `parseMcpOAuthClientCredentials()`／`verifyMcpOAuthClientAuthentication()` 驗證 HTTP Basic 或 POST body 的 client 憑證，修正部分要求 confidential client 的 AI 工具連線失敗（CIMD 仍僅允許 `none`，並在 `validateClientIdMetadataDocument()` 明確拒絕 confidential document）。`app/api/mcp/route.ts` 401 回應改為結構化 `error`/`error_description`/`action: reauthorize`（區分 `invalid_token` 與 `authorization_required`）；`app/oauth/authorize/page.tsx` 的 `ErrorCard` 新增保留 `returnTo` 的重新登入連結；`app/mcp/page.tsx` 補充「正常登出不會撤銷 MCP OAuth 授權」與 confidential client 需走 DCR 的說明。Next.js 16.3 慣例將 `middleware.ts` 遷移為 `proxy.ts`（邏輯不變，`nginx.conf` 註解同步更新）。升級 `next`／`react`／`react-dom`／`resend`／`lucide-react`／`@base-ui/react`／`tailwindcss` 等相依套件至最新相容版本並重新產生 `package-lock.json`（`npm audit` 0 vulnerabilities）。新增/擴充 `tests/lib/mcpOAuth.test.ts` 涵蓋 client credentials 解析、confidential client 驗證、secret 序列化與 401 結構化回應。驗證：`npm run test:mcp-oauth` 14/14、`npm run test:mcp-descriptors`、`npm run typecheck`、`npm test`、`npm run build`、`git diff --check` 均通過。 |
| 4.98.0 | 2026-08-11 | 行動 App 新增管理員設定（`mobile/lib/screens/admin_screen.dart`）：系統設定（開放註冊／LINE 登入／允許註冊 Email／管理員 IP 白名單／稽核模式／交易照片儲存與上限／股價自動更新與間隔，對應 `app/api/admin/system-settings`）、使用者管理（列表／新增／重設密碼／切換管理員／刪除，對應 `app/api/admin/users`）、全站登入稽核（`/api/admin/login-audit?scope=all`），以及維運動作（股價立即更新、交易照片壓縮／加密、伺服器時間、NTP 同步）。設定頁在 `isAdmin` 時顯示管理員入口；`AppUser` 模型新增 `isSuperAdmin`。股票設定畫面（`mobile/lib/screens/stock_settings_screen.dart`）補齊股票狀態管理（下市／恢復追蹤，走 `POST /api/stocks/batch-price` 帶 `delisted`）與定期定額（`/api/stock-recurring` CRUD＋啟用切換，`api_client.dart` `_send` 補 PATCH 支援；`models.dart` 新增 `StockRecurring`）。i18n：45 個 `admin.*` key 注入 10 個 shared ARB（含 10 語翻譯），`npm run i18n:generate` 重產生 web dicts／Flutter 翻譯。驗證：`flutter analyze` 僅既有 3 項 warning；`flutter test` 6/6；`npm test` 15/15；`npm run check:i18n` 通過；`npm run typecheck` 通過。 |
...
| 4.97.2 | 2026-07-30 | 修正 MCP OAuth 授權端點（`app/oauth/authorize/page.tsx` → `lib/mcpOAuth.ts` 的 `fetchPinnedClientMetadata()`）在擷取 Client ID Metadata Document（CIMD）時的連線位址選擇問題。根因：正式環境部署（Zeabur）容器只有 IPv4 對外路由，但 SSRF 防護用的 `dns.promises.lookup(hostname, { all: true, verbatim: true })` 回傳結果可能把 IPv6（AAAA）位址排在前面，原本直接取 `addresses[0]` 連線，若剛好選到 IPv6 位址會立即 `Network unreachable`，導致 client_id 為 CIMD URL 的 MCP 用戶端（如 Claude）在 `/oauth/authorize` 顯示「無法授權 MCP 連線／Unable to retrieve client metadata document」而無法完成授權。以 `npx zeabur@latest service exec` 進入正式環境容器以 `getent ahosts`／`nc` 實測驗證：目標網域確有 AAAA 記錄、對該 IPv6 位址的 TCP 連線確實 `Network unreachable`、IPv4 位址則正常連通。修法：`fetchPinnedClientMetadata()` 在解析出的公開位址中優先挑選 IPv4（`family` 由小到大排序後取第一筆）再建立連線，SSRF 的「僅接受公開 IP」驗證邏輯不變。另確認先前已排查的 `.well-known/oauth-protected-resource`／`.well-known/oauth-authorization-server` 500 錯誤，根因為正式環境未設定 `APP_URL` 環境變數，已由使用者於 Zeabur 後台補上並重啟服務解決，與本次程式碼修正為兩個獨立問題。驗證：`npm run typecheck` 通過；`npm run test:mcp-oauth`（12/12）通過。 |
| 4.97.1 | 2026-07-30 | MCP OAuth 授權端點（`app/api/oauth/authorize/route.ts`）的所有重導向回應（核發 code 成功、使用者拒絕、發生錯誤）新增 `iss` 查詢參數，帶入本服務 issuer（由 `APP_URL` 推導之 origin），符合 RFC 9207（OAuth 2.0 Authorization Server Issuer Identification）與 MCP 規格 `2026-07-28` 版 SEP-2468 建議，供串接用戶端比對預期 issuer 以降低多授權伺服器情境下的 mix-up attack 風險；純新增欄位、向下相容，不影響既有 PAT 或既有已核發之 OAuth token。驗證：`npx tsc --noEmit` 除既有且與本次無關的 e2e Playwright 型別錯誤外無新增錯誤；`npm run test:mcp-oauth`（12/12）與 `npm run test:mcp-auth` 全數通過。 |
| 4.97.0 | 2026-07-29 | MCP 整合新增 OAuth 2.1 授權登入，`/api/mcp` 從僅接受 PAT 擴充為同時接受 PAT（`ap_mcp_`）與 OAuth access token（`ap_mcp_oauth_access_` 前綴）。新增資料表 `mcp_oauth_clients`（`client_id`/`redirect_uris`/`token_endpoint_auth_method` 固定 `none`/`grant_types`/`response_types`/`client_name`/`client_uri`/`logo_uri`/`scope`）、`mcp_oauth_authorization_codes`（`code_hash` 為主鍵、`code_challenge`、`resource`、`expires_at`、`consumed_at` 供 5 分鐘單次使用）、`mcp_oauth_tokens`（`token_hash` 主鍵、`token_type`、`family_id`、`resource`、`expires_at`、`revoked_at`、`replaced_by_hash` 供 refresh token 輪替與重放偵測）。新增 `lib/mcpOAuthCore.ts`（純驗證函式：PKCE S256 驗證/推導、`redirect_uri`／`resource`／scope（固定 `mcp:read`）正規化、Client ID Metadata Document URL 格式檢查、`isPublicMetadataIp()` 阻擋私有/保留 IP 供 CIMD 擷取防護、`isMcpHttpOriginAllowed()` 同源＋`ALLOWED_ORIGINS` allowlist、`resolveMcpOAuthOrigin()` 由 `APP_URL` 推導正式環境 HTTPS origin（本機限定 loopback 才允許 HTTP））與 `lib/mcpOAuth.ts`（`registerMcpOAuthClient()` 動態註冊 RFC 7591／CIMD 兩種來源、`issueMcpAuthorizationCode()`／`exchangeMcpAuthorizationCode()`／`exchangeMcpRefreshToken()`／`verifyMcpOAuthAccessToken()`／`revokeMcpOAuthToken()`，access token 1 小時、refresh token 30 天且每次使用皆輪替並綁定 `family_id`，重放同一 family 即整組撤銷）。新增路由：`app/.well-known/oauth-protected-resource/route.ts` 與 `app/.well-known/oauth-protected-resource/api/mcp/route.ts`（RFC 9728）、`app/.well-known/oauth-authorization-server/route.ts`（RFC 8414，經 `lib/mcpOAuthMetadata.ts` 組裝）、`app/api/oauth/register/route.ts`（RFC 7591 動態註冊）、`app/api/oauth/authorize/route.ts` + `app/oauth/authorize/page.tsx`（沿用既有登入 Cookie 驗證身份後顯示明確同意畫面，`sanitizeClientForConsent()` 僅回傳 client 名稱/圖示/redirect URI 供畫面呈現）、`app/api/oauth/token/route.ts`（`authorization_code`／`refresh_token` 兩種 grant）、`app/api/oauth/revoke/route.ts`。新增 `lib/loginReturn.ts` 的 `safeOAuthReturnTo()`：`returnTo` 參數僅接受同源 `/oauth/authorize` path，其餘一律清空以避免開放重導向；`app/login/page.tsx`、LINE 登入（`app/api/auth/line/authorize/route.ts`、`app/auth/line/callback/page.tsx`、`lib/lineOAuthState.ts`）密碼／Google／Passkey／LINE 四種登入方式皆改為登入成功後導向 `loginDestination()`（預設 `/dashboard`，OAuth 流程中回到授權同意頁）。`app/api/mcp/route.ts` 新增 Origin 驗證（非白名單回 403）、CORS 標頭、`GET`/`OPTIONS`/`DELETE` 方法（原僅 `POST`）、401 回應改為標準 `WWW-Authenticate: Bearer` challenge（含 `resource_metadata` 連結與 `mcp:read` scope，無效權杖另加 `error="invalid_token"`），並以 `lib/mcpOpenAiCompatibility.ts` 的 `OpenAiCompatibleMcpTransport` 包裝既有 transport，補回官方 SDK 1.30 `registerTool()` 會遺漏的 OpenAI 頂層 `securitySchemes` 擴充欄位；`lib/mcpServer.ts` 的 11 個唯讀工具新增 `title`、read-only annotations 與 `mcp:read` OAuth 需求（含 `_meta.securitySchemes` 舊版鏡像）。新增 `app/mcp/page.tsx` 公開服務說明頁，供 metadata 的 `service_documentation` 欄位指向。`middleware.ts` 的 `PUBLIC_PATHS` 新增 `/mcp`、`PUBLIC_PREFIXES` 新增 `/api/oauth/`（略過 Cookie 閘門），新增三組速率限制：`/api/oauth/{authorize,token,revoke}` 每 IP 每 15 分鐘 100 次、`/oauth/authorize` 頁面每 IP 每 15 分鐘 30 次（防止匿名觸發 CIMD 擷取）、`/api/oauth/register` 每 IP 每小時 20 次。`components/features/settings/McpSettingsClient.tsx` 新增 OAuth 連線網址（`/api/mcp`）複製區塊，與既有 PAT 建立/列表/撤銷區塊並存。`.env.example` 的 `APP_URL` 說明更新為「正式環境啟用 MCP OAuth 時必填」。新增 10 語言 i18n 鍵（OAuth 同意畫面、設定頁新區塊文案）。測試：新增 `tests/lib/mcpOAuth.test.ts`（12 項，涵蓋 PKCE、redirect_uri／scope／resource 驗證、Origin allowlist、正式環境未設定 `APP_URL` 時 PAT-only 401 fallback、token 重複參數拒絕、DB 生命週期案例於無 `DATABASE_URL` 時略過）與 `tests/lib/mcpDescriptors.test.ts`（1 項，驗證 `tools/list` 原始回應含頂層 `securitySchemes`／`_meta` 鏡像／read-only annotations），新增 `npm run test:mcp-oauth`／`test:mcp-descriptors` 並納入 `npm test`。驗證：`npm run typecheck` 通過；`npm test` 全數通過（`test:mcp-oauth` 12/12、`test:mcp-descriptors` 1/1、`check:i18n` 10 語言 1,257 鍵對齊）；既有 PAT 連線行為與撤銷流程未變動。 |
| 4.96.0 | 2026-07-29 | 新增 MCP（Model Context Protocol）AI 財務紀錄查詢整合。新增相依套件 `@modelcontextprotocol/sdk@^1.30.0`；新增資料表 `mcp_credentials`（`id`/`user_id`/`name`/`token_hash`/`created_at`/`last_used_at`/`expires_at`/`revoked_at`，唯一索引於 `token_hash`），`lib/mcpAuth.ts` 實作 PAT 產生（`ap_mcp_` + `crypto.randomBytes(32)` base64url）、SHA-256 雜湊索引查找（比照 `lib/userDeletion.ts` 的 `createHashedEmail()` 手法，非 bcrypt，因需雜湊索引查找）、建立（20 組啟用中上限）、驗證、列表（衍生 `active`/`expired`/`revoked` 三態）、撤銷。`lib/mcpServer.ts` 的 `buildMcpServer(credential)` 以官方 SDK `McpServer` 註冊 11 個唯讀工具（`list_transactions`/`get_transactions_summary`/`list_accounts`/`list_categories`/`list_budgets`/`list_recurring`/`list_stock_holdings`/`list_stock_transactions`/`list_stock_dividends`/`list_stock_recurring`/`get_stock_realized_pl`），一律以 `credential.userId` 限定範圍，分頁類工具 `pageSize` 上限 200（超過回傳 `isError: true` 而非空結果）。`app/api/mcp/route.ts` 僅實作 `POST`（`GET`/`DELETE` 回 405），以 `WebStandardStreamableHTTPServerTransport`（`sessionIdGenerator: undefined`）處理無狀態 Streamable HTTP 請求，`Authorization: Bearer <PAT>` 驗證失敗回 401；`middleware.ts` 的 `PUBLIC_PATHS` 新增 `/api/mcp`（略過 Cookie 閘門，全域速率限制與 CSRF 判斷不受影響地自動涵蓋）。查詢邏輯重用既有計算：抽取 `app/api/stock-realized-pl/route.ts` 的 FIFO 已實現損益邏輯為 `lib/stockHelpers.ts` 的 `getStockRealizedPl(userId, { dateFrom?, dateTo?, stockId? })`（無篩選時輸出與抽取前完全一致，篩選僅套用於賣出明細不影響 FIFO 成本計算）；抽取 `app/api/reports/route.ts` 的期間收支彙總為 `lib/dashboardHelpers.ts` 的 `getTransactionsSummary()`；抽取 `app/api/dashboard/route.ts` 內嵌的持股/未實現損益組裝邏輯為同檔的 `getStockPortfolioStatus()`（新增每檔持股股數/均價/現價/未實現損益/幣別明細），三個既有路由皆改為呼叫抽取後的函式，行為不變。新增使用者自助憑證管理：`app/api/user/mcp-credentials/route.ts`（`GET` 列表／`POST` 建立，回傳時間欄位一律轉為 ISO 8601 UTC 字串）、`app/api/user/mcp-credentials/[id]/route.ts`（`DELETE` 撤銷），沿用既有 `authToken` Cookie + `requireAuth()`。新增 `app/settings/mcp/page.tsx` + `components/features/settings/McpSettingsClient.tsx`（建立表單、一次性權杖顯示需確認已複製才可關閉、憑證清單含狀態徽章與撤銷按鈕），`components/layout/Sidebar.tsx` 新增導覽項目。`lib/auditHelpers.ts` 的 `AUDIT_METADATA_ALLOWED_KEYS` 新增 `mcp_credential_id`/`mcp_credential_name`，每次 `tools/call` 成功後寫入 `action: 'mcp_query'` 稽核紀錄（`metadata.scope` 為資料範圍摘要，不含實際金額/明細）。`lib/userDeletion.ts` 的 `USER_OWNED_TABLES` 加入 `mcp_credentials`，帳號刪除同步清除憑證。新增 10 語言 i18n 鍵 `navMcp` + 35 個 `settings.mcp.*` 鍵（`shared/i18n/app_*.arb`，經 `npm run i18n:generate` 產出 Web 字典與 Flutter `gen-l10n` 輸出）；`mobile/` 未納入本次範圍。測試：新增 `tests/lib/mcpAuth.test.ts`／`tests/lib/stockHelpers.test.ts`／`tests/lib/dashboardHelpers.test.ts`（需 `DATABASE_URL`／`POSTGRES_URL`，未設定時自動略過以保持 `npm test` 在無 DB 環境仍可通過；新增 `tests/setup/register.mjs` 提供 Node 原生 ESM loader 解析 `@/*` 別名與省略副檔名的相對匯入，僅供測試執行使用）。驗證：`npm run build`（含 typecheck）通過；`npm test` 全數通過（含 3 組新增 DB 整合測試、`check:iso`、`check:i18n` 10 語言對齊）；以本機 Postgres + dev server 實測完整流程（註冊 → 建立憑證 → 真實 MCP `initialize`/`tools/list`（11 個工具）/`tools/call` → 稽核紀錄寫入 → 撤銷 → 撤銷後立即 401）；`/settings/mcp` 頁面 SSR 輸出以 `curl` 驗證渲染正確；受限於本機無法啟動瀏覽器自動化，前端互動（按鈕點擊、複製剪貼簿、彈窗開關）未做視覺化驗證。 |
| 4.95.2 | 2026-07-28 | 修補 GitHub Dependabot 回報的 10 個 npm 安全性告警（alert #36–#45）。`next` 從 `^16.2.10` 升級至 `^16.2.12`（修補 9 項 16.0.0–16.2.10 範圍內的漏洞，含 Server Actions SSRF、Middleware/Proxy bypass、快取污染、Image Optimization API DoS 等，均由 `16.2.11` 起修補）；`postcss`（`overrides` 與 `devDependencies` 兩處）從 `^8.5.16` 升級至 `^8.5.18`，修補 Source Map 自動載入時的路徑穿越漏洞（可讀取任意 `.map` 檔）。重新產生 `package-lock.json`（`next`/`postcss` 解析至 `16.2.12`/`8.5.23`，5 個套件變動）。驗證：`npm run typecheck` 通過；`npm test` 中 `test:dashboard-forecast`（7 項）與 `check:iso`（15 項）通過；`check:i18n` 的 staleness 檢查在乾淨 main HEAD 上本就會失敗（Windows 本機既有問題，與本次改動無關，已確認回退變更後現象相同）。 |
| 4.95.1 | 2026-07-25 | CI 自動建立 GitHub Release，取代原本 `/update-docs` 收尾時手動 `gh release create` 的步驟（先前只自動打 tag、不建 Release，Releases 頁曾斷在 v4.76.3 與 v4.94.0）。新增 `.github/scripts/release-notes.mjs`：讀 `changelog.json` 指定版本（`VERSION` 未給時取 `currentVersion`），把 `changes` 依 `tag` 分組輸出 Markdown 至 `NOTES_FILE`，區塊順序固定 `warning`（升級需注意）→`new`（新增）→`improved`（改進）→`fixed`（修正）→`removed`（移除），未知 tag 依出現順序接在後面不丟內容，格式比照既有 v4.94.0 的 `### 新增` + bullet；標題以 `vX.Y.Z — <title>` 印到 stdout，changelog 查無該版時退回 `vX.Y.Z` + 一行通用內文。`.github/workflows/docker-publish.yml` 新增 `Publish GitHub Release` step，觸發條件 `github.ref == 'refs/heads/main' | | startsWith(github.ref, 'refs/tags/v')`，tag 觸發時以`GITHUB_REF_NAME` 為準（可補打舊版）、main 觸發時用 `changelog.currentVersion`。刻意排在 Docker Hub／ghcr 推送之後，Release 出現即代表映像檔可拉取，build 失敗則不發。冪等與競態處理：先`git ls-remote` 確認遠端有該 tag（無則只發 `::warning::` 並 `exit 0`）、再`gh release view`存在即略過不覆寫、`gh release create` 失敗但事後 `view` 查得到視為競態成功，其餘情況才 `::error::` + `exit 1`。沿用 job 既有`contents: write` 權限與 runner 內建的 `gh`／`node`，未新增 secret。驗證：抽出 step body 對 stub 過的`git`／`gh` 跑 6 情境（tag 不在遠端／Release 已存在／正常建立／建立撞競態／建立失敗／tag 觸發補打舊版）全數符合預期，腳本另以 4.95.0、4.84.0（含 `warning` tag）、不存在的 9.9.9、省略 `VERSION` 四種輸入實跑，YAML 解析與 `bash -n` 通過。 |
| 4.95.0 | 2026-07-25 | 交易記錄新增「排除轉帳」篩選。`app/api/transactions/route.ts` 的 `GET` 新增 `excludeTransfer` query 參數（接受 `1`／`true`），成立時於 where 條件追加 `AND t.type NOT IN ('transfer_in', 'transfer_out')`，與既有 `type`／`accountId`／`categoryId`／日期／關鍵字條件同層 AND；因 count 與 list 共用同一組 where 與 params，分頁 `total` 與翻頁結果一致。`components/features/transactions/TransactionsClient.tsx` 將 `excludeTransfer` 併入既有 `filters` 狀態（值為 `'1'`／`''`，沿用字串型別以相容 URL 同步與 `JSON.stringify` 比對），因此自動獲得網址同步（`?excludeTransfer=1`）、`activeFilterCount` 計數與 `clearFilters` 還原；`renderFilterFields()` 新增勾選框，桌機 xl grid template 由 `repeat(5,minmax(0,1fr))_auto` 改為 `repeat(5,minmax(0,1fr))_auto_auto` 讓勾選框與「清除」同列。互斥處理：新增 `toggleExcludeTransfer()`，勾選時若 `filters.type === 'transfer'` 則一併重設為空，類型下拉的 `transfer` option 於勾選狀態 `disabled`；`readFilters()` 同步做邊界正規化，手寫網址同時帶 `type=transfer&excludeTransfer=1` 時以排除轉帳優先，避免「已選但永遠空清單」的矛盾狀態。新增 10 語言翻譯鍵 `features.transactions.excludeTransfer`（來源 `shared/i18n/app_*.arb`，經 `npm run i18n:generate` 產出 Web 字典與 Flutter `gen-l10n` 輸出）。行動版 Flutter 交易頁篩選未納入本次範圍。驗證：`npx tsc -p tsconfig.typecheck.json --noEmit` exit 0、`npm test`（含 `check:i18n` 字典對齊與靜態 `t()` 鍵掃描）全過、i18n 產生器重跑輸出穩定（CI drift gate 不觸發）；本機未啟動 dev server 做畫面實測、未跑 `npm run build`。 |
| 4.94.1 | 2026-07-22 | 修補 `Trivy (container scan)` CI 掃到的 HIGH 漏洞 `GHSA-f88m-g3jw-g9cj`（`sharp` 繼承自 libvips 的 CVE-2026-33327/33328/35590/35591）。根因：`next@16.2.10` 自帶 optional dependency `sharp: ^0.34.5`，因 0.x 版號的 caret range 語意（`^0.34.5` 僅涵蓋 `>=0.34.5 <0.35.0`）與專案自身的 `sharp: ^0.35.3` 無法合併，npm 另外裝了一份舊版巢狀套件於 `node_modules/next/node_modules/sharp@0.34.5`，並被 Next.js standalone build 的 tracing 一併打包進最終映像檔（掃描報告路徑為 `app/node_modules/next/node_modules/sharp/package.json`）。專案本身未使用 `next/image`，Next.js 自帶的這份 sharp 純屬未使用的 optional dependency。修法：在 `package.json` 既有的 `overrides` 區塊新增 `"sharp": "^0.35.3"`，強制 npm 全專案僅解析單一份 sharp，消除巢狀舊版拷貝；重新產生 `package-lock.json` 後該巢狀項目與其專屬平台的 `@img/sharp-*` 套件（約 500 行）自 lockfile 移除。驗證：`npm install --package-lock-only` 重生 lockfile 後 `npm ci` 於本機全新安裝成功、0 弱點；Docker 建置與 Trivy 掃描結果待 CI 驗證。 |
| 4.94.0 | 2026-07-22 | 持股總覽的 ETF 卡片新增「配息月份」資訊。`app/api/stocks/route.ts` 的 `GET` 端點在既有 FIFO 持股與市值計算後，新增 `dividendMonths` 衍生欄位：從該股票的 `stock_dividends` 紀錄中篩出 `cash_dividend > 0` 或 `stock_dividend_shares > 0` 的筆數，取 `date` 欄位的月份（`slice(5, 7)`）去重並排序後回傳，完全衍生自使用者既有的配息紀錄，不需另建 ETF 配息月份對照表、也不依賴外部資料源。`components/features/stocks/PortfolioClient.tsx` 新增 `formatDividendMonths()`：以 `Intl.DateTimeFormat`（`month: 'short'`）取得依語系在地化的月份名稱，再用 `Intl.ListFormat`（`style: 'short'`, `type: 'conjunction'`）串接，不支援 `Intl.ListFormat` 的環境 fallback 為逗號串接；僅在 `stockType === 'etf'` 時於卡片標題列下方顯示「配息月份：…」，尚無配息紀錄則顯示提示文字。新增 10 語言 Web 字典鍵 `features.stocks.portfolio.dividendMonths`／`dividendMonthsEmpty`，`node tools/check-i18n-parity.ts` 驗證全對齊、無缺鍵。驗證：`npm run typecheck` 通過；本機未設定 `DATABASE_URL`，無法啟動 dev server 做畫面實測，未跑 `npm run build`。 |
| 4.93.3 | 2026-07-20 | 修正 `/finance/info-board`（滿月資訊版）股票市值每個月都用 `stocks.current_price`（目前股價）重算，導致已結束月份的金額隨當下股價變動的問題（例如 6 月的股票市值會被 7 月最新股價蓋掉）。根因：`lib/fullMoonInfoBoard.ts` 的 `monthlyStockValues()` 對 12 個月一律套用同一個目前股價，沒有區分「已結束月份」與「進行中月份」。修法：新增資料表 `stock_month_close_prices`（`symbol` + `year_month` 為複合主鍵，依股票代號跨使用者共用，不綁定單一使用者）與 `lib/stockMonthClosePrice.ts` 的 `getMonthClosePrices()`：對已結束月份（`index < visibleThroughMonthIndex`）先查快取，未命中才呼叫既有的 `fetchTwseStockDay`/`fetchTpexStockDay`（帶入該月最後一天日期，TWSE/TPEx STOCK_DAY 端點會回傳整月資料，取陣列最後一筆即為當月最後交易日收盤價）並以 `ON CONFLICT(symbol, year_month) DO UPDATE` 回寫快取；`monthlyStockValues()` 已結束月份改用鎖定收盤價，當月（`index === visibleThroughMonthIndex`，尚未結束）仍用 `stocks.current_price` 即時股價，抓不到歷史收盤價時 fallback 回目前股價。`buildStockRows()`／`getFullMoonInfoBoardData()` 改為 async 以支援必要時的即時抓價，`app/finance/info-board/page.tsx` 對應加上 `await`。順手修正 `lib/twseFetchNext.ts` 的 `fetchAllWithLimit<T>` 泛型：原本回傳結果的 `value` 型別誤標成跟輸入 `item` 同型（實際是 fetcher 的回傳值），改為 `fetchAllWithLimit<TItem, TValue>` 兩個獨立型別參數，使新模組不需 `@ts-nocheck` 即可正確通過型別檢查。另補上先前 PR #193／#194 新增的排程現金展望與儲蓄情境試算 shared ARB 翻譯鍵所缺漏的 Flutter `gen-l10n` 產生輸出（`mobile/lib/generated/l10n/app_localizations*.dart`），修正 main 上持續失敗的 i18n 一致性檢查。 |
| 4.93.2 | 2026-07-19 | 修正 `components/features/recurring/RecurringClient.tsx` 的 `openEdit()` 未將 `recurring.amount`（DB 內一律以 TWD 儲存）還原成原幣別金額即填入編輯表單的問題。根因：`app/api/recurring/route.ts` 的 `GET` 回傳的 `amount` 為已換算的 TWD 整數，但 `openEdit()` 直接把該值塞進表單的 `amount` 欄位，同時 `currency`/`fxRate` 仍保留原始外幣設定；使用者若未察覺並直接儲存，`PUT /api/recurring/[id]` 會把畫面上的 TWD 金額當成原幣別金額，再次乘上 `fxRate` 換算，導致金額被壓縮，尤其是 TWD 匯率偏低的幣別（如 JPY、KRW、VND）在反覆編輯幾次後換算結果會 `Math.round()` 為 0，觸發既有的「金額必須為正整數（本幣）」400 驗證錯誤（`app/api/recurring/route.ts:87`、`app/api/recurring/[id]/route.ts:62`）。行動版 `mobile/lib/screens/recurring_screen.dart:247` 已透過 `shown = e.amount / rate` 正確處理此還原邏輯，僅網頁版遺漏。修法：`openEdit()` 比照行動版，當 `currency !== 'TWD'` 且匯率有效時以 `amount / fxRate` 還原原幣別金額並用 `fmtAmountForEdit()` 格式化（整數不留小數，否則取到小數點後兩位）後再填入表單；純 TWD 的固定收支不受影響。 |
