...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  const txs = asRows<AccountTransactionRow>(queryAll(
  ...
  const referenceCount = Number(asRow<{ c: number }>(queryOne(
...
async function updateAccount(request: NextRequest, id: string) {
  ...
  if (newCurrency && newCurrency !== normalizeCurrency(existing.currency)) {
    const refCount = Number(asRow<{ c: number }>(queryOne(
      'SELECT COUNT(*) AS c FROM transactions WHERE (account_id = ? OR to_account_id = ?) AND user_id = ?',
      [id, id, auth.userId]
    ))?.c) || 0;
    if (refCount > 0) {
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  ...
  const count = Number(asRow<{ cnt: number }>(queryOne('SELECT COUNT(*) as cnt FROM accounts WHERE user_id = ?', [auth.userId]))?.cnt) || 0;
