...
function fmtScheduleTime(item: {
  ...
  if (item.freq === "monthly")
    return `${time} (${Number(item.dayOfMonth) === 0 ? "每月最後一天" : `每月 ${item.dayOfMonth} 日`})`;
  return time;
}
...
const AUDIT_META_LABELS: Record<string, string> = {
  ...
  mcp_credential_id: "憑證ID",
  mcp_credential_name: "憑證名稱",
};

const AUDIT_ROLE_VALUE_LABELS: Record<string, string> = {
  user: "一般使用者",
  readonly: "一般管理員",
  super: "超級管理員",
  admin: "管理員",
...
function formatAuditDetail(metadataRaw: unknown): string {
  ...
  Object.keys(meta).forEach((k) => {
    ...
    if ((k === "old_role" || k === "new_role") && typeof v === "string")
      v = AUDIT_ROLE_VALUE_LABELS[v] || v;
...
export default function AdminClient(
  props: { user?: any; isSuperAdmin?: boolean } = {},
) {
  ...
  const [transactionPhotoStorage, setTransactionPhotoStorage] = useState<
    "" | "local" | "s3"
  >("");
  ...
  const [stockAutoUpdateIntervalMin, setStockAutoUpdateIntervalMin] =
    useState("10");
  const [stockAutoUpdateLastRun, setStockAutoUpdateLastRun] = useState(0);
  const [stockAutoUpdateLastSummary, setStockAutoUpdateLastSummary] =
    useState("");
  ...
  const [expenseReminderForm, setExpenseReminderForm] = useState({
    userId: "",
    freq: "daily",
    hour: "21",
    minute: "0",
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [
        ...
        apiGet("/api/admin/login-audit?scope=admin-self").catch(() => ({
          logs: [],
        })),
        apiGet("/api/admin/login-audit").catch(() => ({ logs: [] })),
        apiGet("/api/admin/data-audit/retention").catch(() => ({
          retention_days: "90",
        })),
      ...
      setAllowedEmails(
        Array.isArray(settings.allowedRegistrationEmails)
          ? settings.allowedRegistrationEmails.join("\n")
          : "",
      );
      setIpAllowlist(
        Array.isArray(settings.adminIpAllowlist)
          ? settings.adminIpAllowlist.join("\n")
          : "",
      );
      setTransactionPhotoStorage(
        (settings.transactionPhotoStorage as "" | "local" | "s3") || "",
      );
      setTransactionPhotoMaxMb(
        settings.transactionPhotoMaxBytes
          ? String(Math.round(settings.transactionPhotoMaxBytes / 1024 / 1024))
          : "",
      );
      setPhotoEncryptionEnabled(!!photoStorage?.encryptionEnabled);
      setStockAutoUpdateEnabled(settings.stockAutoUpdateEnabled !== false);
      setStockAutoUpdateIntervalMin(
        String(settings.stockAutoUpdateIntervalMin || 10),
      );
      setStockAutoUpdateLastRun(Number(settings.stockAutoUpdateLastRun) || 0);
      setStockAutoUpdateLastSummary(
        String(settings.stockAutoUpdateLastSummary || ""),
      );
      setUsers(userList || []);
      ...
      setScheduleForm((prev) => ({
        ...prev,
        userId: userList?.[0]?.id || prev.userId || "",
      }));
      setExpenseReminderForm((prev) => ({
        ...prev,
        userId: userList?.[0]?.id || prev.userId || "",
      }));
    } catch (e: any) {
  ...
  const logUserOptions = useMemo(() => {
    ...
    allLogs.forEach((log) => {
      ...
      if (!map.has(email)) {
        map.set(
          email,
          log.displayName ? `${email}（${log.displayName}）` : email,
        );
      }
    });
  ...
  async function saveSystemSettings(e: React.FormEvent) {
    ...
    try {
      ...
      await apiPut("/api/admin/system-settings", {
        publicRegistration,
        lineLoginEnabled,
        allowedRegistrationEmails: allowedEmails
          .split("\n")
          .map((s) => s.trim())
          .filter(Boolean),
        ...
        transactionPhotoMaxBytes:
          transactionPhotoMaxMb === "" ? 0 : Math.round(maxMbNum * 1024 * 1024),
        stockAutoUpdateEnabled,
        stockAutoUpdateIntervalMin: Math.min(
          1440,
          Math.max(1, parseInt(stockAutoUpdateIntervalMin, 10) || 10),
        ),
      });
      setSaveMsg("設定已儲存");
    } catch (e: any) {
  ...
  async function handleStockPriceUpdateNow() {
    ...
    try {
      ...
      if (result?.status === "completed") {
        setStockUpdateMsg(
          `已更新 ${result.updatedSymbols}/${result.totalSymbols ?? result.updatedSymbols} 檔（${result.updatedRows} 筆持股）${result.failed ? `，失敗 ${result.failed} 檔` : ""}`,
        );
      ...
      const settings = await apiGet("/api/admin/system-settings").catch(
        () => null,
      );
      if (settings) {
        setStockAutoUpdateLastRun(Number(settings.stockAutoUpdateLastRun) || 0);
  ...
  async function handleCompressS3Photos() {
    if (
      !confirm(
        "將重新壓縮 S3 上所有尚未壓縮的交易照片，並「原地覆寫」原檔，此動作無法復原。確定執行？",
      )
    )
      return;
    ...
    try {
      ...
      const savedMb = Math.max(
        0,
        (Number(r.bytesBefore) - Number(r.bytesAfter)) / 1024 / 1024,
      );
      setPhotoCompressMsg(
  ...
  async function handleEncryptExistingPhotos() {
    if (
      !confirm(
        "將把所有「尚未加密」的既有交易照片就地加密（本機與 S3），並原地覆寫原檔。確定執行？",
      )
    )
      return;
  ...
  async function handleSetUserRole(
    userId: string,
    role: "user" | "readonly" | "super",
  ) {
    const target = users.find((u) => u.id === userId);
    if (!target) return;
    const currentRole: "user" | "readonly" | "super" = !target.isAdmin
      ? "user"
      : target.adminRole === "readonly" || target.isSuperAdmin === false
        ? "readonly"
        : "super";
    ...
    try {
      await apiPut(`/api/admin/users/${userId}`, {
        isAdmin: role !== "user",
        adminRole: role === "readonly" ? "readonly" : "super",
      });
      await load();
    } catch (e: any) {
      const map: Record<string, string> = {
        ...
        last_super_admin_protected:
          "無法降級：這是系統最後一位超級管理員，降級後將無人能執行變更操作。",
      };
  ...
  function describeRunResult(r: any): string {
    ...
    const detail =
      Array.isArray(r.channels) && r.channels.length
        ? r.channels.join(" / ")
        : r.reason || "";
    switch (r.status) {
      case "completed":
        return `✅ 寄送成功${detail ? `：${detail}` : ""}`;
      case "partial":
        return `⚠️ 部分成功：${detail || "未知"}`;
      case "failed":
        return `❌ 寄送失敗：${detail || "未知錯誤"}`;
      case "skipped":
        return `⏭️ 已略過：${detail || "無"}`;
      default:
        return detail || r.reason || r.status || "排程已執行";
    }
  }
  ...
  async function handleToggleScheduleChannel(
    id: string,
    field: "notifyEmail" | "notifyLine",
    value: boolean,
  ) {
    try {
  ...
  async function handleToggleExpenseReminder(id: string, enabled: boolean) {
    try {
      await apiPut(`/api/admin/line-expense-reminders/${id}`, {
        enabled: !enabled,
      });
      await load();
    } catch (e: any) {
  ...
  async function handleSaveOriginCert() {
    setCertMsg("");
    try {
      await apiPost("/api/admin/certs/origin", {
        cert: originCertPem,
        key: originKeyPem,
      });
  ...
  async function handleDeleteSelectedLogs() {
    ...
    try {
      const result = await apiPost("/api/admin/login-audit/batch-delete", {
        ids: selectedLogIds,
      });
  ...
  async function handleSaveRetention() {
    setAuditMsg("");
    try {
      await apiPut("/api/admin/data-audit/retention", {
    ...
    <div className="space-y-6">
      ...
      {!canWrite && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-800 dark:border-amber-900/60 dark:bg-amber-950/40 dark:text-amber-200">
          ...
          <span>
            您是<strong>一般管理員</strong>，僅具<strong>讀取</strong>
            權限，無法修改設定、匯出資料或調整使用者權限。
          </span>
        </div>
      )}
        ...
        <div className="space-y-6">
            ...
            <form onSubmit={saveSystemSettings} className="space-y-4">
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setPublicRegistration(e.target.checked)
                  }
                  className="w-4 h-4"
                />
                開放第三方服務註冊（Google／LINE）
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={lineLoginEnabled}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setLineLoginEnabled(e.target.checked)
                  }
                  className="w-4 h-4"
                />
                啟用 LINE 登入
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setTransactionPhotoMaxMb(e.target.value)
                  }
                  placeholder={`留空使用環境變數預設值（${Math.round(10485760 / 1024 / 1024)} MB）`}
                />
                <p className="text-xs text-slate-500 mt-1">
                  覆蓋 TRANSACTION_PHOTO_MAX_BYTES，留空則沿用環境變數設定
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                ...
                <div className="flex items-center gap-3">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={handleCompressS3Photos}
                    disabled={photoCompressing || !canWrite}
                  >
                    {photoCompressing ? "壓縮中..." : "壓縮 S3 既有照片"}
                  </Button>
                  {photoCompressMsg && (
                    <span
                      className={`text-sm ${photoCompressMsg.includes("失敗") ? "text-red-500" : "text-slate-600"}`}
                    >
                      {photoCompressMsg}
                    </span>
                  )}
                ...
                <p className="text-xs text-slate-500 mt-1">
                  將 S3 上尚未壓縮的交易照片重新編碼為最長邊 1600px／JPEG
                  82，原地覆寫以節省空間（不可復原）。資料量大時可能需數分鐘。
                </p>
              </div>
              <div>
                  ...
                  <>
                    <div className="flex items-center gap-3">
                      <Button
                        type="button"
                        variant="outline"
                        onClick={handleEncryptExistingPhotos}
                        disabled={photoEncrypting || !canWrite}
                      >
                        {photoEncrypting ? "加密中..." : "加密既有照片"}
                      </Button>
                      {photoEncryptMsg && (
                        <span
                          className={`text-sm ${photoEncryptMsg.includes("失敗") ? "text-red-500" : "text-slate-600"}`}
                        >
                          {photoEncryptMsg}
                        </span>
                      )}
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      新上傳的照片已自動加密。此按鈕會把「尚未加密」的既有照片就地加密（本機與
                      S3），原地覆寫原檔。資料量大時可能需數分鐘。
                    </p>
                  </>
                ) : (
                  <p className="text-xs text-slate-500 mt-1">
              ...
              <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={stockAutoUpdateEnabled}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setStockAutoUpdateEnabled(e.target.checked)
                    }
                    className="w-4 h-4"
                  />
                ...
                <p className="text-xs text-slate-500 mt-1">
                  伺服器於台北時間週一~週五 09:00–14:00
                  內，每隔指定分鐘數自動更新所有使用者的持股現價
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setStockAutoUpdateIntervalMin(e.target.value)
                  }
                  placeholder="10"
                />
                <p className="text-xs text-slate-500 mt-1">
                  1 ~ 1440 分鐘；可用環境變數 STOCK_AUTO_UPDATE_ENABLED /
                  STOCK_AUTO_UPDATE_INTERVAL_MIN 覆寫
                </p>
              </div>
              {saveMsg && (
                <p
                  className={`text-sm ${saveMsg.includes("失敗") ? "text-red-500" : "text-green-600"}`}
                >
                  {saveMsg}
                </p>
              )}
          ...
          <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm space-y-3">
            ...
            <div className="text-sm text-slate-700">
              <div>
                上次更新：
                {stockAutoUpdateLastRun
                  ? new Date(stockAutoUpdateLastRun).toLocaleString()
                  : "尚未執行"}
              </div>
              {stockAutoUpdateLastSummary && (
            ...
            <div className="flex items-center gap-3">
              <Button
                variant="outline"
                onClick={handleStockPriceUpdateNow}
                disabled={stockUpdating || !canWrite}
              >
                {stockUpdating ? "更新中..." : "立即更新股價"}
              </Button>
              {stockUpdateMsg && (
                <span
                  className={`text-sm ${stockUpdateMsg.includes("失敗") ? "text-red-500" : "text-slate-600"}`}
                >
                  {stockUpdateMsg}
                </span>
              )}
          ...
          <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm space-y-4">
            ...
            {serverTimeMsg && (
              <p className="text-sm text-slate-600">{serverTimeMsg}</p>
            )}
          </div>
        </div>
      )}
            ...
            <TableBody>
              {users.map((user) => {
                const role: "user" | "readonly" | "super" = !user.isAdmin
                  ? "user"
                  : user.adminRole === "readonly" || user.isSuperAdmin === false
                    ? "readonly"
                    : "super";
                const roleLabel =
                  role === "user"
                      ...
                      <TableCell className="flex gap-2 flex-wrap items-center">
                        <select
                          ...
                          onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                            handleSetUserRole(
                              user.id,
                              e.target.value as "user" | "readonly" | "super",
                            )
                          }
                          aria-label="設定角色"
                        >
      ...
      {activeTab === "schedules" && (
        <div className="space-y-6">
          <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
            ...
            >
              ...
              <div>
                ...
                >
                  {SCHEDULE_FREQ_OPTIONS.map((option) => (
              ...
              />
              <Button type="submit" className="self-end" disabled={!canWrite}>
                新增排程
              </Button>
              ...
              <div className="md:col-span-4 flex flex-wrap gap-4 text-sm text-slate-700">
                ...
                <label className="inline-flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={scheduleForm.notifyLine}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setScheduleForm((prev) => ({
                        ...prev,
                        notifyLine: e.target.checked,
                      }))
                    }
                  />
                  LINE 通知
            ...
            {scheduleMsg && (
              <p className="text-sm text-slate-600 mt-3">{scheduleMsg}</p>
            )}
          </div>
              ...
              <TableBody>
                {schedules.map((schedule) => (
                  <TableRow key={schedule.id}>
                    <TableCell>
                      {users.find((user) => user.id === schedule.userId)
                        ?.email || schedule.userId}
                    </TableCell>
                    ...
                    <TableCell>
                      <div className="flex flex-wrap gap-2">
                        <button
                          type="button"
                          disabled={!canWrite}
                          className={`rounded border px-2 py-1 text-xs disabled:opacity-50 ${schedule.notifyEmail ? "border-blue-500 text-blue-700" : "border-slate-200 text-slate-400"}`}
                          onClick={() =>
                            handleToggleScheduleChannel(
                              schedule.id,
                              "notifyEmail",
                        ...
                        <button
                          type="button"
                          disabled={!canWrite}
                          className={`rounded border px-2 py-1 text-xs disabled:opacity-50 ${schedule.notifyLine ? "border-green-500 text-green-700" : "border-slate-200 text-slate-400"}`}
                          onClick={() =>
                            handleToggleScheduleChannel(
                              schedule.id,
                              "notifyLine",
          ...
          <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
            ...
            <form
              onSubmit={handleCreateExpenseReminder}
            ...
            >
              <div>
                ...
                <select
                  className="w-full p-2 border rounded-md"
                  value={expenseReminderForm.userId}
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                    setExpenseReminderForm((prev) => ({
                      ...prev,
                      userId: e.target.value,
              ...
              <div>
                ...
                <select
                  className="w-full p-2 border rounded-md"
                  value={expenseReminderForm.freq}
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                    setExpenseReminderForm((prev) => ({
                      ...prev,
                      freq: e.target.value,
              ...
              <Input
                ...
                min={0}
                max={23}
                value={expenseReminderForm.hour}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setExpenseReminderForm((prev) => ({
                    ...prev,
                    hour: e.target.value,
              ...
              <Input
                ...
                min={0}
                max={59}
                value={expenseReminderForm.minute}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setExpenseReminderForm((prev) => ({
                    ...prev,
                    minute: e.target.value,
              ...
              {expenseReminderForm.freq === "weekly" && (
                <Input
                  label="每週星期 (0-6)"
                  type="number"
                  min={0}
                  max={6}
                  value={expenseReminderForm.weekday}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setExpenseReminderForm((prev) => ({
                      ...prev,
                      weekday: e.target.value,
              ...
              {expenseReminderForm.freq === "monthly" && (
                <div>
                  ...
                  <select
                    className="w-full p-2 border rounded-md"
                    value={expenseReminderForm.dayOfMonth}
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                      setExpenseReminderForm((prev) => ({
                        ...prev,
                        dayOfMonth: e.target.value,
                      ...
                      {canWrite ? (
                        <>
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleRunExpenseReminderNow(reminder.id)
                            }
                          >
                            立即提醒
                          </Button>
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleDeleteExpenseReminder(reminder.id)
                            }
                          >
                            刪除
                          </Button>
        ...
        <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm space-y-4">
          ...
          {emailProviders && (
            <div className="grid md:grid-cols-2 gap-4 text-sm text-slate-700">
              ...
              <div>
                SMTP：{emailProviders.configured?.smtp ? "已設定" : "未設定"}
              </div>
              <div>
                Zeabur：
                {emailProviders.configured?.zeabur ? "已設定" : "未設定"}
              </div>
              <div>
                Resend：
                {emailProviders.configured?.resend ? "已設定" : "未設定"}
              </div>
            </div>
          )}
          <Button onClick={handleSendTestEmail} disabled={!canWrite}>
            寄送測試信
          </Button>
                  ...
                  filteredLogs.map((log) => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell>
                        {log.isSuccess
                          ? "成功"
                          : `失敗${log.failureReason ? ` (${log.failureReason})` : ""}`}
                      </TableCell>
                      {canWrite && (
                        <TableCell>
                          <Button
            ...
            <div className="flex flex-wrap gap-3 items-end">
              <div>
                ...
                <select
                  ...
                  onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                    setAuditRetention(e.target.value)
                  }
                  disabled={!canWrite}
                >
              ...
              {canWrite && (
                <>
                  ...
                  <Button variant="destructive" onClick={handlePurgeAudit}>
                    清空稽核日誌
                  </Button>
                </>
              )}
                  ...
                  {auditLogs.map((log) => {
                    ...
                    return (
                      <TableRow key={log.id}>
                        ...
                        <TableCell>
                          {AUDIT_ROLE_VALUE_LABELS[log.role] || log.role}
                        </TableCell>
                        <TableCell title={log.user_agent || ""}>
                          {log.ip_address || "—"}
                        </TableCell>
                      </TableRow>
                    );
        ...
        <div className="space-y-6">
          <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm space-y-4">
            ...
            <div className="text-sm text-slate-600">
              {certInfo?.originCa
                ? `有效期間：${certInfo.originCa.validFrom} ~ ${certInfo.originCa.validTo}`
                : "目前未部署 Origin CA"}
            </div>
            <textarea
              rows={6}
              value={originCaPem}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setOriginCaPem(e.target.value)
              }
              className="w-full p-2 border rounded-md font-mono text-sm"
              placeholder="-----BEGIN CERTIFICATE-----"
            />
            <div className="flex gap-3">
              <Button onClick={handleSaveOriginCa} disabled={!canWrite}>
                部署 Origin CA
              </Button>
              <Button
                variant="destructive"
                onClick={handleDeleteOriginCa}
                disabled={!canWrite}
              >
                刪除 Origin CA
              </Button>
          ...
          <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm space-y-4">
            ...
            <div className="text-sm text-slate-600 space-y-1">
              <p>
                {certInfo?.originCert
                  ? `憑證有效期間：${certInfo.originCert.validFrom} ~ ${certInfo.originCert.validTo}`
                  : "目前未部署 Origin Certificate"}
              </p>
            ...
            <textarea
              ...
              value={originCertPem}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setOriginCertPem(e.target.value)
              }
              className="w-full p-2 border rounded-md font-mono text-sm"
              placeholder="-----BEGIN CERTIFICATE-----"
            />
            <textarea
              ...
              value={originKeyPem}
              onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) =>
                setOriginKeyPem(e.target.value)
              }
              className="w-full p-2 border rounded-md font-mono text-sm"
              placeholder="-----BEGIN PRIVATE KEY-----"
            />
            <div className="flex gap-3">
              <Button onClick={handleSaveOriginCert} disabled={!canWrite}>
                部署 Origin Certificate / Key
              </Button>
              <Button
                variant="destructive"
                onClick={handleDeleteOriginCert}
                disabled={!canWrite}
              >
                刪除全部
              </Button>
