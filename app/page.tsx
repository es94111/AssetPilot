...
function getStats(t: T) {
  return [
    {
      value: t("public.home.stats.modules.value"),
      label: t("public.home.stats.modules.label"),
      sublabel: t("public.home.stats.modules.sublabel"),
    },
    {
      value: t("public.home.stats.encryption.value"),
      label: t("public.home.stats.encryption.label"),
      sublabel: t("public.home.stats.encryption.sublabel"),
    },
    {
      value: t("public.home.stats.stockSource.value"),
      label: t("public.home.stats.stockSource.label"),
      sublabel: t("public.home.stats.stockSource.sublabel"),
    },
    {
      value: t("public.home.stats.precision.value"),
      label: t("public.home.stats.precision.label"),
      sublabel: t("public.home.stats.precision.sublabel"),
    },
  ];
}
...
export default async function Home() {
  ...
  // The login page handles both provider sign-in and provider-based account creation.
  return (
      ...
      <div className="relative mx-auto max-w-6xl px-6 py-8 sm:py-10">
        <header className="flex flex-col gap-5 border-b border-white/10 pb-8 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            <img
              src="/logo.svg"
              alt="AssetPilot"
              className="h-11 w-11 rounded-2xl bg-white p-1.5"
            />
            <div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Link
              href="/login"
              className="rounded-xl border border-white/15 px-4 py-2.5 text-sm font-medium text-slate-100 transition hover:border-white/30 hover:bg-white/5"
            >
              {t("public.home.register")}
            </Link>
          </div>
        </header>
        ...
        <section className="grid gap-10 py-14 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              >
                {t("public.home.startUsing")}
              </Link>
              <Link
                href="/login"
                className="rounded-2xl border border-white/15 px-5 py-3 font-medium text-slate-100 transition hover:border-white/30 hover:bg-white/5"
              >
                {t("public.home.createFirst")}
              </Link>
            </div>
            <ul className="mt-8 flex flex-wrap gap-3 text-sm text-slate-300">
              <li className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.chips.openSource")}
              </li>
              <li className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.chips.encrypted")}
              </li>
              <li className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.chips.noCloudLock")}
              </li>
              <li className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.chips.docker")}
              </li>
              <li className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.chips.openapi")}
              </li>
            </ul>
          </div>
        ...
        <section className="py-6">
          <div className="mb-8">
            <div className="text-sm font-medium uppercase tracking-[0.2em] text-slate-400">
              {t("public.home.whyLabel")}
            </div>
            <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white">
              {t("public.home.whyTitle")}
            </h2>
            <p className="mt-3 max-w-3xl text-slate-300">
              {t("public.home.whyDescription")}
            </p>
          </div>
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            {featurePillars.map((pillar) => (
              <article
                key={pillar.title}
                className={`rounded-3xl border bg-gradient-to-br p-6 ${pillar.accent}`}
              >
        ...
        <section className="grid gap-6 py-14 lg:grid-cols-[1fr_0.9fr] lg:items-start">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
            <div className="text-sm font-medium uppercase tracking-[0.2em] text-slate-400">
              {t("public.home.quickStartLabel")}
            </div>
            <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white">
              {t("public.home.quickStartTitle")}
            </h2>
            <p className="mt-4 max-w-2xl text-slate-300">
              {t("public.home.quickStartDescription")}
            </p>
            <div className="mt-5 flex flex-wrap gap-3 text-sm text-slate-200">
              <span className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.quickStartChips.image")}
              </span>
              <span className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.quickStartChips.arch")}
              </span>
              <span className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.quickStartChips.health")}
              </span>
              <span className="rounded-full border border-white/10 px-3 py-1.5">
                {t("public.home.quickStartChips.keys")}
              </span>
            </div>
          </div>
  ...
  es94111/assetpilot:latest`}</code>
            </pre>
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
          <div className="text-sm font-medium uppercase tracking-[0.2em] text-slate-400">
            {t("public.home.techLabel")}
          </div>
          <h2 className="mt-3 text-3xl font-semibold tracking-tight text-white">
            {t("public.home.techTitle")}
          </h2>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {STACK.map((item) => (
              <span
                key={item}
                className="rounded-full border border-white/10 bg-black/10 px-3 py-2 text-sm text-slate-200"
              >
                {item}
              </span>
            ))}
        ...
        <footer className="flex flex-col gap-4 border-t border-white/10 py-8 text-sm text-slate-400 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <Link href="/api-credits" className="transition hover:text-white">
              {t("public.common.apiCredits")}
            </Link>
          </div>
        </footer>
