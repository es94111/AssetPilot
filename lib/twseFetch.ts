...
export async function fetchAllWithLimit<T, R>(
  items: T[],
  fetcher: (item: T) => Promise<R>
): Promise<Array<{ ok: true; item: T; value: R } | { ok: false; item: T; error: string }>> {
  const limit = Math.max(1, parseInt(process.env.TWSE_MAX_CONCURRENCY || '5', 10) || 5);
  const batches = chunk(items, limit);
  const results: Array<{ ok: true; item: T; value: R } | { ok: false; item: T; error: string }> = [];
  for (const batch of batches) {
    const settled = await Promise.all(batch.map(async (item) => {
      try {
        const value = await fetcher(item);
        return { ok: true as const, item, value };
      } catch (e) {
        return { ok: false as const, item, error: e instanceof Error ? e.message : String(e) };
      }
    }));
...
// - 其餘 → 一般股票
export function inferStockType(symbol: unknown): 'stock' | 'etf' | 'warrant' {
