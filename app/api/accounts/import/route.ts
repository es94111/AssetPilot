...
type AccountImportCellValue =
  ...
  | unknown[];
type AccountImportRow = Record<string, AccountImportCellValue>;
...
function asRows<T>(rows: Array<Record<string, string | number | null>>): T[] {
  // SAFETY: queryAll returns rows from the fixed accounts SELECT list; T is the
  // corresponding domain shape chosen by the caller.
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request
    .json()
    .catch(() => ({}))) as ImportAccountsRequest;
  const { rows } = body;
  ...
  try {
    ...
    const existing = asRows<AccountLookupRow>(
      queryAll("SELECT id, name FROM accounts WHERE user_id = ?", [
        auth.userId,
      ]),
    );
    ...
    rows.forEach((row, idx) => {
      ...
      if (!name) {
        errors.push({ row: idx + 2, reason: "帳戶名稱為空" });
        skipped++;
        return;
      }
      if (byName.has(name)) {
      ...
      const rawAccountType = String(
        cell(row, "accountType", "account_type", "帳戶類型"),
      ).trim();
      ...
      const initialRaw = cell(
        row,
        "initialBalance",
        "initial_balance",
        "初始餘額",
      );
      const initialBalance = initialRaw === "" ? 0 : Number(initialRaw);
      const overseasRaw = cell(
        row,
        "overseasFeeRate",
        "overseas_fee_rate",
        "海外手續費率",
      );
      const overseasFeeRate = overseasRaw === "" ? null : Number(overseasRaw);
      if (
        !Number.isFinite(initialBalance) ||
        (overseasFeeRate != null &&
          (!Number.isFinite(overseasFeeRate) ||
            overseasFeeRate < 0 ||
            overseasFeeRate > 100))
      ) {
        errors.push({
          row: idx + 2,
          reason: "初始餘額與海外手續費率必須為有限且在允許範圍內的數值",
        });
        skipped++;
        return;
      }
      parsed.push({
        ...
        currency: normalizeCurrency(
          String(cell(row, "currency", "幣別") || "TWD"),
        ),
        icon: normalizeAccountIcon(
          String(cell(row, "icon", "圖示") || "fa-wallet"),
        ),
        excludeFromTotal: parseBool(
          cell(row, "excludeFromTotal", "exclude_from_total", "排除總資產"),
        ),
        linkedBankName: String(
          cell(row, "linkedBankName", "linked_bank_name", "連結銀行帳戶"),
        ).trim(),
        overseasFeeRate,
    ...
    if (txStarted) {
      try {
        ...
        console.error("[accounts-import] rollback failed", rollbackError);
      }
    }
    importProgress.set(auth.userId, {
      processed: 0,
