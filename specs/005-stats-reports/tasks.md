...
**Scope**: 3 user story／**26 base FR + 3 sub-FR（`a` 後綴：FR-011a / FR-015a / FR-024a）= 29 FR + OUT-001/002/003/004**／**11 Clarification（4 輪）**／7 SC（其中 SC-007 屬 post-launch retention，不在 build-time 驗證範疇）。
...
- [ ] T015 在 [server.js](../../server.js) `app.get('/api/accounts', ...)`（行 5242）改寫 response 整形：在現有 `balance`（原幣餘額）欄位旁**新增** `twdAccumulated` 欄位（不移除既有 `balance` — 向後相容）；計算邏輯為 `SUM(t.twd_amount × <符號>)` WHERE `account_id = ? AND user_id = ?`，符號規則：`income / transfer_in` 為正、`expense / transfer_out` 為負；**假設 `transactions.type` enum 僅此 4 種**（既有專案 schema 已驗證為此 4 種；若未來新增 `dividend / adjustment` 等類型需於該功能中另行決定符號，本 task 不處理）；外幣帳戶 `initial_balance` 不納入此累計（無對應 twd_amount）；TWD 帳戶因每筆 twd_amount = amount 自然包含 `initial_balance` 對應的歷史交易（若 `initial_balance != 0` 但無對應交易，前端需了解此值不會反映於 twdAccumulated；以系統慣例多數使用者初始餘額用一筆 income 交易記錄，故無實質落差）。同 PR 須更新 [contracts/stats-reports.openapi.yaml](./contracts/stats-reports.openapi.yaml#components/schemas/AccountWithTwdAccumulated) 與根目錄 [openapi.yaml](../../openapi.yaml) 之 `Account` schema 加入 `twdAccumulated` 欄位（憲章 Principle II 規則 #2）。對應 FR-004、FR-025、Round 4 釐清；屬 Foundational 因 US1（T027）需依賴此欄位（**無 [Story] 標籤**，因為定位為跨 user story 共用基礎設施 — 雖然目前僅 US1 消費，但 contract 修改屬於跨頁基礎契約變更）。
...
- [ ] T049 [US2] 在 [app.js](../../app.js) 新增（或改寫既有）`renderTrendLine(dailyMap, from, to)` 折線圖：(a) 計算期間天數 `daysCount = diffDays(from, to)`；(b) 採以下**閾值表**（複製自 [research.md §9](./research.md)，inline 於本 task 以利實作者自封閉）：
...
- [ ] T050 [US2] 在 [app.js](../../app.js) 新增（或改寫既有）`renderDailyBar(dailyMap, from, to)` 長條圖：與 T049 採完全相同的閾值表（建議共用 `buildAggregationGranularity(daysCount)` helper 回傳 `'daily' | 'weekly' | 'monthly'`）；空 dailyMap 即呼叫 `renderEmptyState`。對應 FR-014、FR-015。
