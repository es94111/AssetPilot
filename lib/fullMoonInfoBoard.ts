...
function asRows<T>(rows: Array<Record<string, string | number | null>>): T[] {
  // SAFETY: SQL row aliases are defined by each caller's SELECT and only project scalar database values.
...
function asRow<T>(
  ...
  // SAFETY: SQL row aliases are defined by the caller's SELECT and the nullable result is preserved.
...
function accountCreatedAfterMonth(
  account: AccountRow,
  endDate: string,
): boolean {
  const createdAt =
...
function accountCategory(account: AccountRow): string {
  return (
    account.category || categoryFromAccountType(account.account_type || "")
  );
}
...
function monthlyAccountBalances(
  userId: string,
  account: AccountRow,
  year: number,
  visibleThroughMonthIndex: number,
  ...
  const txs = asRows<TransactionRow>(
    queryAll(
  ...
  return FULL_MOON_MONTHS.map((_, index) => {
    ...
    const balance = txs
      ...
      .reduce(
        (current, tx) =>
          applyTransactionToBalance(current, tx, accountCurrency, userId),
...
function buildAccountRows(
  ...
  year: number,
  visibleThroughMonthIndex: number,
): { assets: BoardRow[]; debts: BoardRow[] } {
  const accounts = asRows<AccountRow>(
    queryAll(
  ...
  for (const account of accounts) {
    ...
    const balances = monthlyAccountBalances(
      userId,
      account,
      year,
...
function buildCategoryRows(
  ...
  const rows = asRows<CategoryTransactionRow>(
    queryAll(
  ...
  for (const row of rows) {
    ...
    if (!buckets.has(key)) {
      buckets.set(key, {
        ...
        values: emptyValues(),
        parentOrder: Number(
          row.cat_parent_sort_order ?? row.cat_sort_order ?? 9999,
        ),
  ...
  return Array.from(buckets.values())
    ...
    .map((row) => ({
      group: row.group,
      item: row.item,
      values: blankMonthsAfter(
        row.values.map(roundTwd),
        visibleThroughMonthIndex,
      ),
      tone:
        type === "expense" &&
        /投資|股票|基金|ETF/i.test(`${row.group} ${row.item}`)
          ? "investment"
          : undefined,
    }));
}
...
function monthlyStockValues(
  ...
  const txs = asRows<StockTransactionRow>(
    queryAll(
  ...
  const currentPrice =
    Number(stock.current_price) > 0 ? Number(stock.current_price) : 0;
  ...
  return FULL_MOON_MONTHS.map((_, index) => {
    ...
    const lockedPrice =
      String(stock.market || "TW").toUpperCase() === "TW" &&
      index < visibleThroughMonthIndex
...
async function buildStockRows(
  ...
  const stocks = asRows<StockRow>(
    queryAll(
      `SELECT id, symbol, market, name, current_price, currency
     FROM stocks
  ...
  const closedMonthIndexes = Array.from(
    { length: visibleThroughMonthIndex },
    (_, index) => index,
  );
  const monthClosePrices = await getMonthClosePrices(
    stocks
      .filter((stock) => String(stock.market || "TW").toUpperCase() === "TW")
...
function buildCashDividendRow(
  userId: string,
  year: number,
  visibleThroughMonthIndex: number,
): BoardRow | null {
  const dividends = asRows<StockDividendRow>(
    queryAll(
      `SELECT sd.date, sd.cash_dividend, s.currency
     FROM stock_dividends sd
     JOIN stocks s ON s.id = sd.stock_id AND s.user_id = sd.user_id
  ...
  for (const dividend of dividends) {
    ...
    if (index === null) continue;
    values[index] +=
      (Number(dividend.cash_dividend) || 0) *
      getExchangeRateToTwd(userId, normalizeCurrency(dividend.currency));
  }
  if (!values.some((value) => value !== 0)) return null;
  return {
    group: "",
    item: "現金股利",
    values: blankMonthsAfter(values.map(roundTwd), visibleThroughMonthIndex),
  };
}

function currentPeriodForUser(userId: string): {
  year: number;
  visibleThroughMonthIndex: number;
} {
  const user = asRow<{ timezone: string | null }>(
    queryOne("SELECT timezone FROM users WHERE id = ?", [userId]),
  );
...
export async function getFullMoonInfoBoardData(
  userId: string,
): Promise<FullMoonInfoBoardData> {
  ...
  const stockRows = await buildStockRows(
    userId,
    year,
    visibleThroughMonthIndex,
  );
  ...
  const cashDividendRow = buildCashDividendRow(
    userId,
    year,
    visibleThroughMonthIndex,
  );
  ...
  return {
    ...
    visibleThroughMonthIndex,
    sections,
    hasRecordedData: sections.some((section) =>
      section.rows.some((row) => row.values.some((value) => value !== 0)),
    ),
  };
}
