...
| 版本    | 日期       | 變更說明                                                                                                                                                                                                                                                         |
| ------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.49.1  | 2026-03-24 | 修正匯率設定「新增幣別」按鈕：新增列不再預設為不可編輯的 TWD，改為可直接輸入 3 碼幣別代碼的空白列                                                                                                                         |
| 3.49    | 2026-03-24 | 匯率設定支援自訂幣別：可輸入任意 3 碼幣別代碼，並在交易與帳戶幣別選單即時使用；匯率儲存改為同步清單，移除幣別後不會自動回補                                                                                                                         |
| 3.48    | 2026-03-24 | 全站配色系統一致化：登入頁、圖表與提示元件改用一致主色語系，並同步補齊深色模式語義色對應                                                                                                                         |
| 3.47.3  | 2026-03-24 | 重新設計圓餅圖與雙圓餅圖配色，採用更沉穩一致的主題色系，並以父分類主色與子分類同色系深淺階梯色呈現                                                                                                                         |
| 3.47.2  | 2026-03-24 | 統一儀表板與統計報表圓餅圖/雙圓餅圖圖例順序，固定為父分類總額高到低，再依同父分類下子分類總額高到低                                                                                                                         |
| 3.47.1  | 2026-03-24 | 統一儀表板與統計報表的單圓餅圖與雙圓餅圖配色規則，使圖表風格與網站主題一致，並維持父分類/子分類層級辨識                                                                                                                         |
| 3.47    | 2026-03-23 | 儀表板「支出分類」與「資產配置（含股票市值）」新增雙圓餅圖開關（內圈父分類、外圈子分類），並優化雙圓餅圖配色辨識度                                                                                                                         |
| 3.46.2  | 2026-03-23 | 美化統計報表雙圓餅圖配色，提升父分類與子分類之間的視覺區隔與辨識度                                                                                                                         |
| 3.46.1  | 2026-03-23 | 修正統計報表雙圓餅圖內圈父分類提示標題顯示，避免誤用「父分類 > 子分類」字樣                                                                                                                         |
| 3.46    | 2026-03-23 | 收支管理的統計報表新增「雙圓餅圖」切換：內圈顯示父分類、外圈顯示子分類，並同步強化分類摘要呈現                                                                                                                         |
| 3.45    | 2026-03-23 | 帳號設定新增「修改顯示名稱」：可直接編輯並儲存顯示名稱，更新後即時套用於帳號資訊與側邊欄                                                                                                                         |
| 3.44    | 2026-03-23 | 交易記錄支援未來交易：可新增與編輯未來日期交易，列表新增「未來」標籤，並提供「未來交易」快速篩選                                                                                                                         |
| 3.42.2  | 2026-03-23 | 管理員登入紀錄刪除新增備援重試，降低偶發 502 導致的操作中斷，提升刪除穩定性                                                                                                                                                                                     |
| 1.0     | 2026-03-18 | 初版草稿                                                                                                                                                                                                                                                         |
...
| 3.42    | 2026-03-23 | 登入稽核新增 IP 國家查詢：整合 ipinfo.io API，管理員與個人登入紀錄顯示國家代碼，並加入快取與內網位址 LOCAL 辨識                                                                                                                               |
| 3.42.1  | 2026-03-23 | 修正管理員登入紀錄刪除 404：新增舊版刪除路由相容處理與前端 404 自動同步容錯，避免刪除時跳出伺服器暫時異常訊息                                                                                                                              |
| 3.42.2  | 2026-03-23 | 管理員登入紀錄刪除新增備援重試：主路由失敗時自動改走相容路徑，降低偶發 502 對操作的影響                                                                                                                                                |
| 3.42.3  | 2026-03-23 | 補齊 IPinfo 資料來源標註：API 使用與授權頁及文件新增 `IP address data is powered by IPinfo` 與 `https://ipinfo.io/lite` 連結                                                                                                              |
| 3.43    | 2026-03-23 | 全部使用者登入紀錄新增手動同步：管理員頁可獨立同步全站登入紀錄，並顯示上次同步時間                                                                                                                                                |
### 8.3 未來擴充方向
