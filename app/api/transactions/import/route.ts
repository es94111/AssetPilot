...
type ImportCellValue =
  ...
  | unknown[];
type ImportTransactionRow = Record<string, ImportCellValue>;
...
function asRows<T>(rows: Array<Record<string, string | number | null>>): T[] {
  // SAFETY: queryAll rows are selected from the fixed categories/accounts schema;
  // callers provide the corresponding domain row type for those SELECT lists.
...
function asRow<T>(
  ...
  // SAFETY: queryOne rows are selected from a fixed schema and the caller's
  // generic describes exactly those selected columns.
  return row as unknown as T | null;
}

function cell(row: ImportTransactionRow, ...keys: string[]): ImportCellValue {
...
export async function POST(request: NextRequest) {
  ...
  const body = (await request
    .json()
    .catch(() => ({}))) as ImportTransactionsRequest;
  ...
  try {
    ...
    if (unknownColumnsSet.size > 0) {
      console.log(
        JSON.stringify({
          event: "csv_unknown_columns",
          userId: auth.userId,
          action: "import_transactions",
          columns: [...unknownColumnsSet],
        }),
      );
    }
    ...
    const categories = asRows<CategoryLookupRow>(
      queryAll("SELECT * FROM categories WHERE user_id = ?", [auth.userId]),
    );
    const accounts = asRows<AccountLookupRow>(
      queryAll("SELECT * FROM accounts WHERE user_id = ?", [auth.userId]),
    );
    ...
    accounts.forEach((a) => {
      accMap[String(a.name)] = a;
    });

    const existingTx = asRows<ExistingTransactionRow>(
      queryAll(
    ...
    if (autoCreate) {
      ...
      rows.forEach((row) => {
        ...
        if (dbType && category && !catMap[categoryName]) {
          ...
          colorIdx++;
          db.run(
            "INSERT INTO categories (id, user_id, name, type, color, is_default, sort_order) VALUES (?,?,?,?,?,0,?)",
        ...
        if (account && !accMap[accountName]) {
          const accId = uid();
          db.run(
            "INSERT INTO accounts (id, user_id, name, initial_balance, icon, currency) VALUES (?,?,?,0,'fa-wallet','TWD')",
    ...
    rows.forEach((row, idx) => {
      ...
      const currency =
        String(cell(row, "currency", "幣別") || "TWD").trim() || "TWD";
      const originalRaw = cell(
        row,
        "originalAmount",
        "original_amount",
        "原始金額",
      ...
      const fxFeeRaw = cell(row, "fxFee", "fx_fee", "匯兌手續費");
      const transferToAccount = cell(
        row,
        "transferToAccount",
        "transfer_to_account",
      ...
      const excludeFromStats = parseBool(
        cell(row, "excludeFromStats", "exclude_from_stats", "排除統計"),
      );
      const date =
        typeof rawDate === "string" && isValidIso8601Date(rawDate)
          ? rawDate
          : normalizeDate(String(rawDate || ""));
      const amt = Number(amount);
      const originalAmount = originalRaw === "" ? amt : Number(originalRaw);
      const twdAmount = twdAmountRaw === "" ? 0 : Number(twdAmountRaw);
      const fxFee = fxFeeRaw === "" ? 0 : Number(fxFeeRaw);
      ...
      if (
        ...
        originalAmount < 0 ||
        !Number.isFinite(twdAmount) ||
        twdAmount < 0 ||
        !Number.isFinite(fxFee) ||
        fxFee < 0
      ) {
        errors.push({
          row: idx + 2,
          reason: "原始金額、台幣金額與手續費必須為有限且不可為負的數值",
        });
        skipped++;
        return;
      }
      if (twdAmountRaw !== "" && Math.abs(twdAmount - amt) > 0.001) {
        errors.push({ row: idx + 2, reason: "台幣金額必須與交易金額一致" });
        skipped++;
        return;
      }
      const fxRateNumber = Number(fxRate);
      if (!Number.isFinite(fxRateNumber) || fxRateNumber <= 0) {
        errors.push({ row: idx + 2, reason: "匯率必須為有限且大於 0 的數值" });
        skipped++;
        return;
      }
      ...
      parsedRows.push({
        ...
        transferToAccountId,
        tags,
        excludeFromStats: excludeFromStats ? 1 : 0,
      });
    });
    ...
    if (txStarted) {
      try {
        ...
        console.error("[transactions-import] rollback failed", rollbackError);
      }
    }
    importProgress.set(auth.userId, {
      processed: 0,
    ...
    writeOperationAudit({
      ...
      metadata: {
        ...
        failure_reason: String(e instanceof Error ? e.message : e).slice(
          0,
          200,
        ),
      },
