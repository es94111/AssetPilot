...
export async function POST(request) {
  ...
  if (!stockId || !date) return NextResponse.json({ error: '必填欄位未填' }, { status: 400 });

  const cash = Number(cashDividend) || 0;
  const stkDivShares = Number(stockDividendShares) || 0;
  if (cash < 0 || stkDivShares < 0) return NextResponse.json({ error: '股利不可為負' }, { status: 400 });
  if (cash === 0 && stkDivShares === 0) {
    return NextResponse.json({ error: '現金股利與股票股利至少填一項' }, { status: 400 });
  }

  const stock = queryOne('SELECT id, name FROM stocks WHERE id = ? AND user_id = ?', [stockId, auth.userId]);
  if (!stock) return NextResponse.json({ error: '股票不存在' }, { status: 400 });

  if (cash > 0 && !accountId) {
    return NextResponse.json({ error: '入款帳戶為必填（含現金股利時）' }, { status: 400 });
  }
  if (accountId) {
  ...
  db.run(
    ...
    [id, auth.userId, stockId, date, cash, stkDivShares, accountId || null, note || '', Date.now()]
  );
  ...
  if (stkDivShares > 0) {
    ...
    db.run(
      ...
      [synthTxId, auth.userId, stockId, date, 'buy', stkDivShares, 0, 0, 0, null, synthNote, Date.now(), 1]
    );
  }
  saveDB();
