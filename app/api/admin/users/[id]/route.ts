...
export async function PUT(request, { params }) {
  ...
  const nextAdminRole = nextIsAdmin
    ? String(body?.adminRole || "").toLowerCase() === "readonly"
      ? "readonly"
      : "super"
    : "super";

  const user = queryOne(
    "SELECT id, email, is_admin, admin_role FROM users WHERE id = ?",
    [id],
  );
  if (!user)
    return NextResponse.json({ error: "使用者不存在" }, { status: 404 });
  const oldRole = !user.is_admin
    ? "user"
    : String(user.admin_role || "super").toLowerCase() === "readonly"
      ? "readonly"
      : "super";
  ...
  if (user.is_admin && !nextIsAdmin) {
    const adminCount =
      queryOne("SELECT COUNT(*) AS cnt FROM users WHERE is_admin = 1")?.cnt ||
      0;
    if (adminCount <= 1) {
      return NextResponse.json(
  ...
  const db = getDB();
  db.run("UPDATE users SET is_admin = ?, admin_role = ? WHERE id = ?", [
    nextIsAdmin ? 1 : 0,
    nextAdminRole,
    id,
  ]);
  ...
  const updated = queryOne(
    "SELECT id, email, display_name, created_at, google_id, line_id, is_admin, admin_role FROM users WHERE id = ?",
    [id],
  );
  const updatedIsSuper =
    !!updated.is_admin &&
    String(updated.admin_role || "super").toLowerCase() !== "readonly";
  return NextResponse.json({
    id: updated.id,
    email: updated.email,
