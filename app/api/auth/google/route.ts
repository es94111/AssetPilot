...
const GOOGLE_OAUTH_REDIRECT_URIS = (
...
function isAllowedGoogleRedirectUri(uri: string) {
  ...
  return (
    googleRedirectUriAllowlist.has(stripped) ||
    googleRedirectUriAllowlist.has(stripped + "/")
  );
}
...
export async function POST(request: Request) {
  ...
  if (getTurnstileSiteKey()) {
    ...
    if (!turnstile.ok) {
      ...
      return NextResponse.json(
        { error: turnstile.error || "請先完成真人驗證" },
        { status: 403 },
      );
    }
  }
  ...
  if (!GOOGLE_CLIENT_SECRET)
    return NextResponse.json(
      { error: "Google SSO 需設定 GOOGLE_CLIENT_SECRET" },
      { status: 400 },
    );
  ...
  if (integrityToken && isPlayIntegrityConfigured()) {
    ...
    const expectedNonce = consumeIntegrityNonce(integrityNonce)
      ? integrityNonce
      : undefined;
    ...
    if (!result.ok && isPlayIntegrityEnforced()) {
      return NextResponse.json(
        { error: "裝置完整性驗證未通過，請於 Google Play 安裝的官方 App 重試" },
        { status: 403 },
      );
    }
  }
  ...
  try {
    ...
    const redirectCandidates = originalRedirect
      ? [
          ...new Set([
            originalRedirect,
            originalRedirect.endsWith("/")
              ? originalRedirect.slice(0, -1)
              : originalRedirect + "/",
          ]),
        ]
      : [""];
    ...
    if (!user) {
      ...
      db.run(
        "INSERT INTO users (id, email, password_hash, display_name, google_id, avatar_url, is_admin, has_password, created_at) VALUES (?,?,?,?,?,?,?,?,?)",
        [
          id,
          email,
