...
| 版本 | 日期 | 變更說明 |
| --- | --- | --- |
| 4.37.3 | 2026-05-08 | 安全驗證差異補齊：①Next.js `requireAuth` / `requireServerAuth` 補回 JWT `tokenVersion` 與 `users.token_version` 比對，登出、修改密碼、管理員重設密碼後舊 Token 立即失效；②middleware 與 API auth helper 補回 Cookie 認證狀態變更請求的 Origin / Referer CSRF 檢查；③新增 `lib/envSecrets.ts`，啟動時載入 `ENV_PATH` 並在缺少 `JWT_SECRET` / `DB_ENCRYPTION_KEY` 時自動產生寫入，避免固定預設 JWT 密鑰與未加密資料庫；④`/api/account/settings/delete` 與 `/api/account/settings/password` 復用主要帳號端點，避免 Google-only 刪除帳號與修改密碼流程安全性分岔；⑤production CSP 移除 `unsafe-eval`，僅 dev mode 保留 |
