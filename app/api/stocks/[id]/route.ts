...
export async function PUT(request, { params }) {
  ...
  const s = queryOne("SELECT * FROM stocks WHERE id = ? AND user_id = ?", [
    id,
    auth.userId,
  ]);
  ...
  if (typeChanged) {
    ...
    historicalSells.forEach((t) => {
      ...
      const newTax = calcStockTax(amount, type, settings, market);
      db.run("UPDATE stock_transactions SET tax = ? WHERE id = ?", [
        newTax,
        t.id,
      ]);
      recalculated += 1;
...
export async function DELETE(request, { params }) {
  ...
  const { id } = await params;
  const s = queryOne("SELECT id FROM stocks WHERE id = ? AND user_id = ?", [
    id,
    auth.userId,
  ]);
  if (!s) return NextResponse.json({ error: "股票不存在" }, { status: 404 });
  ...
  const db = getDB();
  db.run("DELETE FROM stock_transactions WHERE stock_id = ? AND user_id = ?", [
    id,
    auth.userId,
  ]);
  db.run("DELETE FROM stock_dividends WHERE stock_id = ? AND user_id = ?", [
    id,
    auth.userId,
  ]);
