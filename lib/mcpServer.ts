...
import {
  ...
  createWriteOAuthToolDescriptor,
  createUpdateOAuthToolDescriptor,
} from "./mcpOpenAiCompatibility";
import {
  normalizeCurrency,
  convertToTwd,
  normalizeDate,
...
function resolvePagination({ page, pageSize }: PaginationInput): {
  ...
  const p =
    Number.isFinite(page) && Number(page) >= 1 ? Math.floor(Number(page)) : 1;
  const ps =
    Number.isFinite(pageSize) && Number(pageSize) >= 1
...
function parseStoredJson<T = Record<string, unknown>>(value: unknown): T {
  try {
    ...
    throw new Error(
      `已儲存的冪等回應無效：${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
const paginationShape = {
  ...
  pageSize: z
    ...
    .min(1)
    .optional()
    .describe(`單頁筆數，預設 ${DEFAULT_PAGE_SIZE}，上限 ${MAX_PAGE_SIZE}`),
};
...
export function buildMcpServer(credential: VerifyMcpTokenResult): McpServer {
  ...
  server.registerTool(
    ...
    async ({
      ...
      if (keyword) {
        where += " AND LOWER(t.note) LIKE LOWER(?)";
        params.push(`%${keyword}%`);
      }
  ...
  server.registerTool(
    "get_transactions_summary",
    {
      ...
      description:
        "查詢指定期間的收支彙總（依分類彙總、每日/每月彙總、總計），不受筆數上限",
      inputSchema: {
    ...
    async ({ yearMonth, page, pageSize }) => {
      ...
      return withAudit(credential, "budgets_list", () => {
        ...
        const total =
          Number(
            queryOne(
              `SELECT COUNT(*) AS cnt FROM budgets WHERE ${where}`,
              params,
            )?.cnt,
          ) || 0;
        const rows = queryAll(
          `SELECT * FROM budgets WHERE ${where} ORDER BY year_month DESC LIMIT ? OFFSET ?`,
          [...params, ps, offset],
        );
        const items = rows.map((b) => {
          ...
          if (b.category_id) {
            ...
            usedParams.push(b.category_id as string);
          }
    ...
    async ({ stockId, dateFrom, dateTo, type, page, pageSize }) => {
      ...
      return withAudit(credential, "stock_transactions_list", () => {
        const total =
          Number(
            queryOne(
              `SELECT COUNT(*) AS cnt FROM stock_transactions st WHERE ${where}`,
              params,
            )?.cnt,
          ) || 0;
        const rows = queryAll(
          `SELECT st.*, s.symbol, s.market, s.currency, s.name AS stock_name
    ...
    async ({ stockId, dateFrom, dateTo, page, pageSize }) => {
      ...
      return withAudit(credential, "stock_dividends_list", () => {
        const total =
          Number(
            queryOne(
              `SELECT COUNT(*) AS cnt FROM stock_dividends sd WHERE ${where}`,
              params,
            )?.cnt,
          ) || 0;
        const rows = queryAll(
          `SELECT sd.*, s.symbol, s.market, s.currency, s.name AS stock_name
  ...
  server.registerTool(
    "get_credit_card_repayment_preview",
    {
      ...
      description:
        "在送出信用卡還款前，依指定付款帳戶與總金額試算各卡的分配結果與還款後預估餘額；本工具不建立任何紀錄。",
      inputSchema: {
        fromAccountId: z
          .string()
          .describe("付款帳戶 id（須為本人所有，且不可為信用卡帳戶）"),
        totalAmount: z
    ...
    async ({ fromAccountId, totalAmount }) => {
      ...
      const fromCurrency = normalizeCurrency(
        fromAccount.currency as string | null,
      );
      ...
      if (numTotal < cards.length) {
        throw new Error(
          `金額過小，至少需 ${cards.length} 才能讓每張卡都分配到`,
        );
      }
      ...
      return withAudit(credential, "credit_card_repayment_preview", () =>
        toolResult({
          fromAccountId,
  ...
  server.registerTool(
    "list_credit_card_repayments",
    {
      ...
      description:
        "查詢過去已建立的信用卡總金額還款紀錄（分頁、可依日期篩選），包含每筆的付款帳戶、日期、總金額與各卡實際分配明細；若該筆還款事後被刪除或修改，會明確標示「已與現況不符」。",
      inputSchema: {
        dateFrom: z
          .string()
          .optional()
          .describe("起始日期 YYYY-MM-DD（含當日），依還款日期篩選"),
        dateTo: z
          .string()
          .optional()
    ...
    async ({ dateFrom, dateTo, page, pageSize }) =>
      withAudit(credential, "credit_card_repayments_list", () => {
        ...
        const totalRow = queryOne(
          `SELECT COUNT(*) AS cnt FROM credit_card_repayment_summaries WHERE ${where}`,
          params,
        );
  ...
  if (credential.allowCreate === true) {
    server.registerTool(
      "create_transaction",
      {
        ...
        description:
          "新增一筆一般交易紀錄（收入、支出或轉帳），視同使用者本人手動建立；僅能新增，無法修改或刪除既有資料。",
        inputSchema: {
          type: z
            .string()
            .optional()
          ...
          amount: z
            .union([z.number(), z.string()])
            .optional()
            .describe("交易金額，必須大於 0（以 currency 欄位所指定的幣別計）"),
          currency: z
            .string()
            .optional()
            .describe("ISO 4217 幣別碼，預設 'TWD'"),
          fxRate: z
            .number()
            .optional()
            .describe("手動指定匯率；未提供時自動查匯率；type=transfer 時忽略"),
          date: z
            .string()
            .optional()
            .describe("交易日期 YYYY-MM-DD；未提供時為使用者所屬時區的今天"),
          categoryId: z
            .string()
            .optional()
            .describe(
              "分類 id（僅 income/expense 有意義；找不到本人所屬分類時視為未分類，不拒絕建立）",
            ),
          accountId: z
            .string()
            .optional()
            .describe(
              "帳戶 id（income/expense 用；使用者僅一個帳戶時可省略，自動代入）",
            ),
          fromAccountId: z
            .string()
            .optional()
            .describe("轉出帳戶 id（type=transfer 時必填，須為本人所有）"),
          toAccountId: z
            .string()
            .optional()
            .describe(
              "轉入帳戶 id（type=transfer 時必填，須為本人所有，不可與 fromAccountId 相同）",
            ),
          note: z
            .string()
            .optional()
            .describe("備註全文；會存入交易本身，但不會寫入稽核日誌"),
          idempotencyKey: z
            .string()
            .optional()
            .describe(
      ...
      async ({
        ...
        if (type === "transfer") {
          ...
          const toCurrency = normalizeCurrency(
            toAccount.currency as string | null,
          );
          if (fromCurrency !== toCurrency) {
            throw new Error(
              "CrossCurrencyTransfer：跨幣別請分開記一筆支出＋一筆收入",
            );
          }
          ...
          response = {
            transferOut: pair.transferOut,
            transferIn: pair.transferIn,
          };
        ...
        return withAudit(
          ...
          {
            transaction_id: transactionIdForAudit,
    ...
    server.registerTool(
      "create_credit_card_repayment",
      {
        ...
        description:
          "以一個總金額代使用者送出信用卡還款，視同使用者本人在網頁版或行動 App 操作；系統會依各卡目前欠款自動等比例分配，無需逐張提供金額。",
        inputSchema: {
          fromAccountId: z
            .string()
            .describe(
              "付款帳戶 id（須為本人所有，且不可為信用卡帳戶，可由 list_accounts 取得）",
            ),
          totalAmount: z
            .union([z.number(), z.string()])
            .describe("本次還款總金額，須為大於 0 的整數（以付款帳戶幣別計）"),
          date: z
            .string()
            .optional()
            .describe("還款日期 YYYY-MM-DD；未提供時為使用者所屬時區的今天"),
          idempotencyKey: z
            .string()
            .optional()
            .describe(
      ...
      async ({ fromAccountId, totalAmount, date: rawDate, idempotencyKey }) => {
        ...
        return withAudit(
          ...
          {
            repayment_summary_id: summaryId,
            transaction_summary: `信用卡還款 ${numTotal} 元`,
          },
        );
      },
    );
  ...
  if (credential.allowUpdateNote === true) {
    server.registerTool(
      "update_transaction_note",
      {
        ...
        description:
          "更新一筆既有交易紀錄的備註文字，視同使用者本人手動編輯備註；僅能修改備註，無法變更日期、類型、分類、金額、帳戶，也無法刪除任何資料。",
        ...
        inputSchema: {
          transactionId: z
            .string()
            .describe(
              "要更新備註的交易 id（須為本人名下既有交易，可由 list_transactions 取得）",
            ),
          note: z
            .string()
      ...
      async ({ transactionId, note }) => {
        ...
        const editBlock = findTransactionEditBlock({
          is_fx_fee: row.is_fx_fee as number | null,
        });
        if (editBlock) {
