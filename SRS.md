...
- **描述：** 系統可自動定期備份資料，並提供使用者在設定頁手動備份與還原
- **處理規則：**
  - 伺服器依設定週期（預設每 6 小時）自動建立資料庫備份
  - 自動清理舊備份，僅保留最近 N 份（預設 30 份）
  - 使用者可自訂自動備份週期與保留份數，儲存後立即套用
  - 使用者可在「設定 > 資料匯出匯入」查看備份清單（時間、檔名、大小）
  - 使用者可手動立即建立備份
  - 使用者可下載指定備份檔至本機
...
| 方法 | 端點                        | 說明                           |
| ---- | --------------------------- | ------------------------------ |
| GET  | /api/system/backups         | 取得備份清單與自動備份設定資訊 |
| PUT  | /api/system/backups/settings | 更新自動備份間隔與保留份數     |
