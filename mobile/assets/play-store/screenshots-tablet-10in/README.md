# Google Play 10 吋平板截圖

依 `mobile/GOOGLE_PLAY_GRAPHIC_GUIDE.md` 產生的 8 張 10 吋平板上架截圖，畫面與真實
...
來源在 `mobile/assets/play-store/_build/`（與手機 / 7" 平板共用 `style.js` / `gen.js`）。
10" 平板用 720×1280 邏輯像素 + CSS `zoom:2.5`（DSF=1）渲染成 1800×3200。

> 為何用 `zoom` 而非 `--force-device-scale-factor`：Edge headless 在 DSF 縮放路徑下
> 對較高的頁面有點陣化異常；`zoom` + DSF=1 可得到單一乾淨點陣圖。

```powershell
# 1) 產生 10" 平板 HTML（720x1280 + zoom 2.5）
cd mobile/assets/play-store/_build
node gen.js tablet10

# 2) 用 Edge headless 渲染成 1800x3200 PNG（DSF=1）
$edge = "C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
$dir  = "..\screenshots-tablet-10in"
Get-ChildItem $dir -Filter *.html | ForEach-Object {
  ...
  Start-Process $edge -Wait -NoNewWindow -ArgumentList `
    ...
    "--force-device-scale-factor=1","--window-size=1800,3200",
    "--screenshot=$png",$url
}
```

> 其他尺寸：手機 1080×1920 在 `../screenshots/`（`node gen.js`）、7" 平板 1440×2560 在
> `../screenshots-tablet-7in/`（`node gen.js tablet`）。App 畫面若有調整，請同步更新
> `gen.js` 後重新產生各尺寸。
