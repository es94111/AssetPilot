...
// Base UI owns focus restoration, Escape handling and aria-modal semantics.
// Keep the wrapper small so every feature uses the same interaction contract.
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Popup
        ...
        className={cn(
          "fixed top-1/2 left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-popover p-4 text-sm text-popover-foreground ring-1 ring-foreground/10 shadow-[var(--shadow-glass-lg)] duration-150 outline-none data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95 sm:max-w-sm",
          className
        )}
        {...props}
      >
...
function DialogDescription({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
