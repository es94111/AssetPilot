...
<p align="center">
  <img src="https://img.shields.io/badge/version-4.29.0-blue" alt="version">
...
- **排程寄送**：多筆排程並存（每日 / 每週 / 每月）；以伺服器時區固定觸發；SMTP 失敗自動 fallback Resend；停用期間漏掉的觸發點不溯及補寄
...
- **SMTP / Resend 設定**：管理員 UI 內直接編輯 SMTP（Host / Port / User / Password / From / TLS）；環境變數設定 Resend
...
| 圖表 | Chart.js |
| 寄信 | Resend / Nodemailer SMTP（即時切換、執行期 fallback） |
...
> 💡 **SMTP**（Gmail / Outlook 等）**不走環境變數**。請至「管理員 → SMTP 寄信設定」UI 直接輸入，即時生效。SMTP 設定值會優先於 Resend，失敗時會自動 fallback Resend。
