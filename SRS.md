...
| 版本 | 日期 | 變更說明 |
| --- | --- | --- |
| 5.0.1 | 2026-05-05 | 修正 Docker standalone 模式 `instrumentation.js` 路徑解析錯誤（`./lib` → `process.cwd()/lib`）導致容器啟動即失敗；修正 `data_operation_audit_log` 定期清理 SQL 使用錯誤欄位名稱（`recorded_at` → `timestamp`）導致清理從未執行；`package.json` / `package-lock.json` 版本號同步至 `5.0.0`；OpenAPI 契約 cookie 名稱修正為 `authToken` |
| 5.0.0 | 2026-05-05 | 前端框架遷移至 Next.js 15 App Router + React 19 + Tailwind CSS v4（feature 010）：所有頁面改為 React 元件，API 路由改為 Next.js Route Handlers（路徑與回應格式向後相容），部署改為 standalone 模式（`node .next/standalone/server.js`），Docker 改多階段建置；新增 `lib/scheduler.js`（排程邏輯由 `server.js` 拆出）、`instrumentation.js`（Next.js 15 stable API，DB init + 稽核定期清理）；`next.config.js` 加入 Windows non-ASCII 路徑 `fs.readlink` EINVAL→ENOENT patch、靜態資源長期快取 headers；所有 128 個既有測試 pass |
