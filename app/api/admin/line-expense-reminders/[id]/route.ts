...
export async function PUT(request, { params }) {
  ...
  getDB().run(`UPDATE line_expense_reminders SET ${cols}, updated_at = ? WHERE id = ?`, vals);
  saveDB();
  const updated = queryOne('SELECT * FROM line_expense_reminders WHERE id = ?', [id]);
...
export async function DELETE(request, { params }) {
  ...
  if (!row) return NextResponse.json({ error: '提醒不存在' }, { status: 404 });
  getDB().run('DELETE FROM line_expense_reminders WHERE id = ?', [id]);
  saveDB();
