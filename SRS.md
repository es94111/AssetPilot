...
| 版本 | 日期 | 變更說明 |
| --- | --- | --- |
| 4.52.1 | 2026-06-11 | 容器安全修補：`Dockerfile` runner 階段加入 `RUN apk upgrade --no-cache`，升級 base image（`node:24-alpine`）落後的 OS 套件至 alpine repo 最新修補版，修補 Trivy 掃描回報的 OpenSSL `libcrypto3`/`libssl3` 高風險弱點（PKCS7_verify use-after-free，CVE-2026-45447），讓 `trivy.yml`（`ignore-unfixed: true`、HIGH/CRITICAL `exit-code: 1`）的容器掃描通過。無應用程式碼變更 |
