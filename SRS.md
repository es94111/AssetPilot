...
| 版本 | 日期 | 變更說明 |
| --- | --- | --- |
| 4.104.0 | 2026-08-22 | Dashboard Web 進行 editorial financial cockpit 視覺改版：`app/dashboard/page.tsx` 保留既有資料查詢、模組個人化、期間篩選與所有 drill-down 連結，新增 `dashboard-page`／`dashboard-hero--editorial` 等 scoped class，於頁首顯示查詢時間狀態；`app/globals.css` 新增工作區專用 tonal panel、ruled canvas、serif data、mono metadata 與 dark navy hero token，workspace Dashboard 不再依賴 glass/blur；新增 `--pl-gain`／`--pl-loss` 語意 token，Portfolio unrealized P/L loss 改用中性 orange 而非 danger red。未修改 Dashboard API contract、資料計算、i18n key、schema 或 dependency。驗證：`npm run typecheck`、Dashboard targeted tests、`npm run build`、`git diff --check` 通過；`npm test` 於既有 generated i18n freshness check 失敗，無新增測試失敗。 |
