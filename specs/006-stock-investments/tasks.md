...
- [ ] T011a 在 [server.js](../../server.js) `initDatabase()` 內 T011 後續新增 ALTER：`try { db.run("ALTER TABLE stock_transactions ADD COLUMN tax_auto_calculated INTEGER DEFAULT 1"); } catch (e) {}`（冪等模式）。`tax_auto_calculated` 欄位用於 FR-001 末段「修改持股類型重算」時判定哪些歷史 sell 交易 tax 可被覆寫（=1 自動計算可覆寫；=0 使用者手動輸入須保留）。對應 [data-model.md §2.3](./data-model.md)、FR-001、U1 / S1 補強。
- [ ] T012 在 [server.js](../../server.js) `initDatabase()` 內 T011a 後續新增 partial unique index：`db.run("CREATE UNIQUE INDEX IF NOT EXISTS idx_stock_tx_recurring_idem ON stock_transactions (user_id, recurring_plan_id, period_start_date) WHERE recurring_plan_id IS NOT NULL AND period_start_date IS NOT NULL")`。對應 [data-model.md §2.4](./data-model.md)、Pass 3 Q5。
...
- [ ] T032 [US1] 在 [server.js](../../server.js) `/api/stocks` GET handler T031 後續：每檔 stock response 補欄位：`delisted: !!s.delisted`、`lastQuotedAt: s.updated_at`（沿用既有 `updated_at`，不新增 schema 欄位）、`priceSource`（**由 T018 強化後的 TWSE helper 直接 propagate 實際使用端點**；若 baseline 尚未保留 source 資訊，T018 audit 階段 MUST 補上回傳 source 的能力，**不允許**在本 task 內基於當前時間猜測 priceSource，避免誤標 stale 資料的來源 — A1 修正後規範）；下市股票（`delisted = 1`）priceSource 固定為 `'frozen'`。對應 FR-004、FR-005、Pass 1 Q5、A1 補強。
