...
import { fetchUsDividends } from "../../../../lib/usStockFetch";
...
async function fetchTwseDividendList(startDate, endDate) {
  ...
  if (
    dividendCache[key] &&
    now - dividendCache[key].timestamp < DIVIDEND_CACHE_TTL
  ) {
...
export async function POST(request) {
  ...
  try {
    ...
    if (
      yearParam !== null &&
      (!Number.isInteger(yearParam) || yearParam < 2010 || yearParam > 2099)
    ) {
      return NextResponse.json(
        { error: "year 參數必須為 2010-2099" },
        { status: 400 },
      );
    }

    const stocks = queryAll("SELECT * FROM stocks WHERE user_id = ?", [
      auth.userId,
    ]);
    if (stocks.length === 0)
      return NextResponse.json({ synced: 0, skipped: 0, errors: [] });

    const twSymbolSet = new Set(
      stocks
        .filter((s) => normalizeStockMarket(s.market) === "TW")
        .map((s) => s.symbol),
    );
    ...
    stocks.forEach((s) => {
      ...
      if (periods.length > 0)
        stockHoldingPeriods[stockMarketKey(s.market, s.symbol)] = {
          stock: s,
          txs,
          periods,
        };
    ...
    if (yearParam !== null) {
      ...
      maxYear = minYear;
      Object.values(stockHoldingPeriods)
        .filter(({ stock }) => normalizeStockMarket(stock.market) === "TW")
        .forEach(({ periods }) => {
          periods.forEach((p) => {
    ...
    let allDividends = [];
    if (twSymbolSet.size > 0) {
      for (let y = minYear; y <= maxYear; y++) {
        const sd = `${y}0101`;
        const ed = y === maxYear ? today.replace(/-/g, "") : `${y}1231`;
        const divs = await fetchTwseDividendList(sd, ed);
        allDividends = allDividends.concat(
          divs.map((d) => ({ ...d, market: "TW" })),
        );
        if (y < maxYear) await delay(2000);
      }
    }

    for (const holding of Object.values(stockHoldingPeriods)) {
      if (normalizeStockMarket(holding.stock.market) !== "US") continue;
      const starts = holding.periods
        .map((p) => p.start)
        .filter(Boolean)
        .sort();
      if (starts.length === 0) continue;
      const usStart = yearParam === null ? starts[0] : `${yearParam}-01-01`;
      const usEnd =
        yearParam === null || yearParam === Number(today.slice(0, 4))
          ? today
          : `${yearParam}-12-31`;
      const usDividends = await fetchUsDividends(
      ...
      allDividends.push(
        ...usDividends.map((d) => ({
          ...
          date: d.date,
          valuePerShare: d.amountPerShare,
          type: "息",
          detailKey: `US:${holding.stock.symbol}:${d.date}`,
        })),
      );
    }

    const relevantDivs = allDividends.filter(
      (d) => stockHoldingPeriods[stockMarketKey(d.market, d.symbol)],
    );
    const seenKeys = new Set();
    const uniqueDivs = relevantDivs.filter((d) => {
      const key = d.detailKey || `${d.market}:${d.symbol},${d.date}`;
    ...
    for (const div of uniqueDivs) {
      const holding =
        stockHoldingPeriods[stockMarketKey(div.market, div.symbol)];
      if (!holding) {
        skipped++;
        continue;
      }
      ...
      const syncMarker =
        div.market === "US" ? "Yahoo Finance自動同步" : "TWSE自動同步";
      const existing = queryOne(
        "SELECT id FROM stock_dividends WHERE user_id = ? AND stock_id = ? AND (date = ? OR (date LIKE ? AND note LIKE ?))",
        [auth.userId, stock.id, div.date, divMonth + "%", `%${syncMarker}%`],
      );
      if (existing) {
        skipped++;
        continue;
      ...
      let cashPerShare = 0,
        stockPer1000 = 0;
      if (div.market === "US") {
        cashPerShare = Number(div.valuePerShare) || 0;
      } else if (div.type === "息") {
        ...
        if (detail) {
          ...
          if (div.type === "權") {
            skipped++;
            errors.push(`${div.symbol} ${div.date} 無法取得除權明細`);
            continue;
          }
          cashPerShare = div.valuePerShare;
        }
      ...
      const cashPrecision = div.market === "US" ? 100 : 1;
      const cashDividend =
        Math.round(sharesHeld * cashPerShare * cashPrecision) / cashPrecision;
      const stockDividendShares =
        stockPer1000 > 0
          ? Math.round(((sharesHeld * stockPer1000) / 1000) * 100) / 100
          : 0;
