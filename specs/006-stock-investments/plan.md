...
- **Baseline 未實作（本計畫補強）**：
  ...
  19. FR-001 末段類型修改重算 — 為 `stock_transactions` 加 `tax_auto_calculated` 欄位（DEFAULT 1），重算僅覆寫自動計算的歷史 sell tax，保留手動覆寫值（U1 / S1 補強）。
  20. FR-006 TPEX 上櫃 fallback audit — 確認 TWSE TSE 失敗時 fall through 至 TPEX 對應端點（G1 補強）。
  21. FR-028 TWT49U / TWT49UDetail 30 分鐘 in-memory cache audit / 補強（G2 補強）。
  22. SC-001 / SC-002 / SC-003 效能驗證 — DevTools Performance / Network 量測並截圖留證（G4 補強）。
...
1. **Schema 補強**（[data-model.md](./data-model.md)）：
   ...
   - 為 `stock_transactions` 加 `tax_auto_calculated INTEGER DEFAULT 1` 欄位（標示稅額是自動計算或手動覆寫；FR-001 末段觸發類型修改重算時用於判定哪些歷史 sell 交易的 tax 值可被覆寫；見 [data-model.md §2.3](./data-model.md)）。
   - 為 `stock_transactions` 補一個「股票股利合成交易」識別方式：採用 **`note LIKE '[SYNTH] 股票股利%'` 唯一前綴**（U3 修正後規範；避免使用者輸入碰撞）；spec 視為「`type='buy'` + `price=0` + 備註以 `[SYNTH] 股票股利` 開頭」。
...
- **[II] OpenAPI 3.2.0 契約 Gate**：
  - 本計畫新增端點：
    ...
    - `GET /api/stocks/quote`（TWSE 即時查價代理；FR-007 三段失敗分流）
    - `GET /api/stock-transactions`（補伺服器端分頁與搜尋，FR-017；本端點若 baseline 已存在則屬 query/response shape 擴充，已記錄於本計畫 Complexity Tracking 豁免一同處理）
    - `GET /api/stock-dividends`（補伺服器端分頁與搜尋，FR-017；同上備註）
  - 修改既有端點：
...
index.html                                   # 新增「實現損益」Tab 容器：
                                             #  - 於股票頁區塊插入 <div id="page-stock-realized-pl"></div>
                                             #  - 新增對應 Tab 切換 button（沿用既有 .tab-btn 樣式）
                                             #  其餘區塊不變動，最小擾動原則。
...
|-----------|------------|-------------------------------------|
| **`/api/stocks` GET response-shape change（array → `{stocks, portfolioSummary}`）以 MINOR bump（`4.26.0 → 4.27.0`）出貨，未依憲章 Principle II Rule #3 升 MAJOR。** | 本專案為單體應用：唯一的 API consumer 是同 repo 內的 `app.js`（前端 SPA），於同 PR 一併修改以匹配新 response shape；無外部第三方 client、無公開 API 訂閱者、亦無歷史 SDK 在外散布。憲章 Governance §4 允許「於 PR 中記錄豁免理由」作為合規路徑，本條目即為該豁免記錄。 | **MAJOR bump（`4.26.0 → 5.0.0`）**：被拒絕，因為 SemVer MAJOR 通常代表「面向外部 client 的破壞性切版」（如 Docker tag / Git tag / CHANGELOG release notes 觸發訂閱通知）；對純內部 API 加上 MAJOR 會導致 release notes 噪音與 ops 流程過度反應，且本 PR 的「frontend + backend bundled change」已透過代碼審查確保整合一致。**保留 array 並新增 sibling endpoint（如 `/api/stocks/portfolio-summary`）**：被拒絕，因為前端需要持股清單與彙總同時渲染，新 endpoint 會增加 round-trip 與資料一致性視窗（兩次 fetch 之間若有交易寫入會出現偏差）；而本 PR 的根本目的就是讓總覽卡與持股卡片同 fetch。 |

**生效範圍**：此豁免僅適用於本 PR（006-stock-investments）的 `/api/stocks` GET response-shape change 一項；未來若本端點再有 breaking change 仍須走 MAJOR bump（或記錄獨立豁免）。其餘新端點（`/api/stocks/batch-fetch`、`/api/stock-realized-pl`、`/api/stock-dividends/sync` 等）皆為純新增、不觸發此豁免。
