...
| 變數 | 說明 | 預設值 |
| ---- | ---- | ------ |
| `SSL_CERT` | Cloudflare Origin Certificate 路徑 | — |
| `SSL_KEY` | Cloudflare Origin Certificate 私鑰路徑 | — |

> 💡 Origin Certificate 可透過管理員面板上傳，但需重啟伺服器才能套用。

**📧 寄信服務（選配，用於管理員寄送資產統計報表）**

| 變數 | 說明 | 預設值 |
| ---- | ---- | ------ |
| `RESEND_API_KEY` | [Resend](https://resend.com/api-keys) API Key | — |
| `RESEND_FROM_EMAIL` | Resend 寄件人，須為已驗證網域信箱，格式 `Name <addr@domain>` | — |
| `APP_URL` | 對外網址，用於信件 CTA「前往儀表板」按鈕（留空則隱藏） | — |

> 💡 **SMTP（如 Gmail / Outlook）不需設環境變數**，請在「管理員 → SMTP 寄信設定」UI 直接輸入 Host / Port / User / Password / From / TLS，設定即時生效不需重啟。SMTP 設了會優先於 Resend。
...
| `SSL_CERT` | Cloudflare Origin Certificate 路徑 | — |
| `SSL_KEY` | Cloudflare Origin Certificate 私鑰路徑 | — |
| `RESEND_API_KEY` | Resend 寄信 API Key（管理員寄送資產統計報表用） | — |
| `RESEND_FROM_EMAIL` | Resend 寄件人（須為已驗證網域信箱） | — |
| `APP_URL` | 對外網址，用於信件 CTA 按鈕（留空則隱藏） | — |

> 💡 **SMTP 寄信設定**（Host / Port / User / Password / From / TLS）改在「管理員 → SMTP 寄信設定」UI 設定，**不走環境變數**。SMTP 設了優先於 Resend。
