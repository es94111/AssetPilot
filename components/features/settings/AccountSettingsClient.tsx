...
function isMessageError(message: string) {
  return /\u5931\u6557|\u932f\u8aa4|\u683c\u5f0f|\u4e0d\u53ef|\u8acb|failed|invalid|required|cannot|please/i.test(
    message,
  );
}
...
export default function AccountSettingsClient({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDefaultCurrency(
        String(u.defaultCurrency || u.default_currency || "TWD").toUpperCase(),
      );
    } catch (_) {}
    setLoading(false);
  }, []);
  ...
  async function handleDisplayName(e: React.FormEvent) {
    e.preventDefault();
    if (!displayName.trim()) {
      setDnMsg(ta("messages.displayNameRequired"));
      return;
    }
    setDnSaving(true);
    setDnMsg("");
    try {
      await apiPut("/api/account/display-name", {
        displayName: displayName.trim(),
      });
      setDnMsg(ta("messages.displayNameUpdated"));
      await load();
    } catch (e: any) {
      setDnMsg(e.message || ta("messages.updateFailed"));
    }
    setDnSaving(false);
  }
  ...
  async function handleTheme(mode: string) {
    ...
    setThemeSaving(true);
    try {
      await apiPut("/api/account/theme", { themeMode: mode });
    } catch (_) {}
  ...
  async function handleDefaultCurrency(e: React.FormEvent) {
    ...
    setCurrencyMsg("");
    try {
      const res = await apiPut("/api/user/settings/default-currency", {
  ...
  async function handleRegisterPasskey() {
    ...
    try {
      const { key, challenge } = (await apiGet(
        "/api/account/passkey/challenge",
      )) as { key: string; challenge: string };
      const deviceName =
        navigator.userAgent.includes("iPhone") ||
        navigator.userAgent.includes("iPad")
          ? "iPhone/iPad"
          : navigator.userAgent.includes("Android")
            ? ta("messages.androidDevice")
            : ta("messages.computerDevice");
      ...
      setPkError(e.message || ta("messages.passkeyRegisterFailed"));
    }
  }
  ...
  async function handleLinkLine() {
    ...
    try {
      ...
      if (!cfg?.lineChannelId || !cfg?.lineCodeFlow)
        throw new Error(ta("messages.lineNotConfigured"));
    ...
    <div className="space-y-6">
      ...
      <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
        ...
        <div className="flex gap-4 mb-2">
          <span className="text-slate-600 w-24">{ta("email")}</span>
          <span className="font-medium">{profile?.email || "—"}</span>
        </div>
        <div className="flex gap-4">
          <span className="text-slate-600 w-24">{ta("displayName")}</span>
      ...
      <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
        ...
        <form onSubmit={handleDisplayName}>
          ...
          {dnMsg && (
            <p
              className={`text-sm mt-2 ${isMessageError(dnMsg) ? "text-red-500" : "text-green-600"}`}
            >
              {dnMsg}
            </p>
          )}
          <Button type="submit" className="mt-4" disabled={dnSaving}>
            {dnSaving ? ta("saving") : ta("updateName")}
          </Button>
        </form>
      </div>
      ...
      <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
        ...
        <form onSubmit={handleDefaultCurrency} className="space-y-3 max-w-xs">
          ...
          {currencyMsg && (
            <p
              className={`text-sm ${isMessageError(currencyMsg) ? "text-red-500" : "text-green-600"}`}
            >
              {currencyMsg}
            </p>
          )}
          <Button type="submit" disabled={currencySaving}>
            {currencySaving ? ta("saving") : ta("updateDefaultCurrency")}
          </Button>
        </form>
      </div>
      ...
      <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
        ...
        {!pkLoading && passkeys.length === 0 && (
          <p className="text-slate-500 text-sm">{ta("noPasskeys")}</p>
        )}
          ...
          {passkeys.map((pk: any) => (
            ...
            >
              ...
              <Button
                variant="ghost"
                size="icon"
                onClick={() => handleDeletePasskey(pk.credId || pk.id)}
        ...
        <Button className="mt-4" onClick={handleRegisterPasskey}>
          {ta("addPasskey")}
        </Button>
      </div>
      ...
      <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
        ...
        <p className="text-sm text-slate-500 mb-4">
          {ta("statusPrefix")}
          {profile?.googleLinked ? ta("linkedGoogle") : ta("notLinkedGoogle")}
        </p>
        <div className="flex gap-3 flex-wrap">
          {!profile?.googleLinked && (
            <Button onClick={handleLinkGoogle}>{ta("linkGoogle")}</Button>
          )}
          {profile?.googleLinked && (
        ...
        {googleMsg && (
          <p className="text-sm text-slate-600 mt-3">{googleMsg}</p>
        )}
      </div>
      ...
      <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
        ...
        <p className="text-sm text-slate-500 mb-4">
          {ta("statusPrefix")}
          {profile?.lineLinked ? ta("linkedLine") : ta("notLinkedLine")}
        </p>
        <div className="flex gap-3 flex-wrap">
          {!profile?.lineLinked && (
            <Button onClick={handleLinkLine} disabled={lineLoading}>
              {lineLoading ? ta("lineVerifying") : ta("linkLine")}
            </Button>
          )}
          {profile?.lineLinked && (
            <Button variant="outline" onClick={handleUnlinkLine}>
              {ta("unlink")}
            </Button>
          )}
        </div>
      ...
      <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
        ...
        {sessionsMsg && (
          <p
            className={`text-sm mb-3 ${isMessageError(sessionsMsg) ? "text-red-500" : "text-green-600"}`}
          >
            {sessionsMsg}
          </p>
        )}
                ...
                {sessions.map((session: any) => (
                  <tr key={session.id} className="border-b last:border-0">
                    <td className="py-3 pr-4 font-medium">
                      {session.deviceName || ta("unknownDevice")}
                      {session.isCurrent ? ta("currentDeviceSuffix") : ""}
                    </td>
                    <td className="py-3 pr-4">
                      {new Date(Number(session.loginAt) || 0).toLocaleString(
                        dateLocale,
                      )}
                    </td>
                    <td className="py-3 pr-4">
                    ...
                    <td className="py-3">
                      <Button
                        variant="outline"
                        onClick={() =>
                          handleLogoutSession(session.id, !!session.isCurrent)
                        }
                      >
                        {ta("signOut")}
                      </Button>
      ...
      <div className="p-6 bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl shadow-sm">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="outline" onClick={loadLoginAudit}>
            {ta("refresh")}
          </Button>
        </div>
        {auditLoading ? (
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {loginAudit.map((log, index) => (
                  <tr
                    key={`${log.loginAt}-${index}`}
                    className="border-b last:border-0"
                  >
                    <td className="py-3 pr-4">
                      {new Date(Number(log.loginAt) || 0).toLocaleString(
                        dateLocale,
                      )}
                    </td>
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button
              ...
              disabled={deleting}
            >
              {deleting ? ta("deleting") : ta("deletePermanently")}
            </Button>
          </div>
        </div>
      </Modal>
