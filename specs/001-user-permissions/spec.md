...
#### 註冊與登入
- **FR-001**：系統必須允許訪客以電子郵件與密碼註冊帳號，前提是該帳號尚未被使用、電子郵件格式正確且密碼符合強密碼規則。
- **FR-002**：系統必須強制密碼至少 8 字元，且同時包含大寫字母、小寫字母、數字、特殊符號。
- **FR-003**：系統必須在註冊成功後自動登入、建立預設分類（含子分類）與一個名為「現金」的預設帳戶。
- **FR-004**：系統必須使用 JWT Token 並存放於 `HttpOnly`、`Secure`、`SameSite=Strict` 的 Cookie；有效期由環境變數 `JWT_EXPIRES` 控制。
- **FR-005**：系統必須在登出時清除 Cookie 並遞增資料庫端 `token_version`，使尚未過期的相同 Token 立即失效。
- **FR-006**：登入 API 必須在回應中附上本次登入的稽核紀錄（`currentLogin`），讓前端可立即顯示。
- **FR-007**：系統必須對 `/api/auth/login`、`/api/auth/register`、`/api/auth/google`、`/privacy`、`/terms` 套用速率限制（每 IP 每 15 分鐘 20 次）。

#### Google SSO
- **FR-010**：系統必須僅在後端設定 `GOOGLE_CLIENT_ID` 時於登入頁顯示 Google 登入按鈕；否則完全隱藏，不影響其他流程。
- **FR-011**：系統必須以 OAuth Authorization Code Flow 完成 Google 登入，並在授權前由後端發行一次性 `state`，於回呼時驗證。
...
- **FR-021**：系統必須允許使用者在登入頁以 Passkey 完成無密碼登入，不需再輸入帳密。
...
- **FR-032**：白名單非空時，一般註冊與 Google 首次註冊皆須比對白名單；不在名單內者拒絕建立帳號。
- **FR-033**：白名單為空且公開註冊關閉時，訪客不得自行註冊；此時僅管理員可直接建立帳號。
- **FR-034**：系統必須允許管理員新增使用者（可選是否授與管理員身分）、重設任一使用者密碼、刪除任一使用者。
- **FR-035**：刪除使用者時，系統必須連帶刪除其所有交易、帳戶、分類、預算、固定收支、股票標的、股票交易、股利、Passkey、Google 綁定、登入稽核紀錄。
...
- **FR-044**：管理員的兩種紀錄清單必須支援單筆刪除、多選批次刪除、手動同步、並顯示上次同步時間。
...
- **FR-062**：系統必須啟用 CSP、HSTS、X-Content-Type-Options、Referrer-Policy 等安全標頭，並停用 `X-Powered-By`。
- **FR-063**：任何外部 CDN 腳本必須附加 SRI `integrity` 屬性。
- **FR-064**：`.env` 檔案權限必須為 `0o600`；資料庫備份檔必須列入 `.gitignore` 與 `.dockerignore`，不得進入版本控制或 Docker Image。
...
- **SC-007**：註冊政策切換後，訪客註冊行為最多在 60 秒內生效（含快取失效）。
- **SC-008**：刪除使用者後，以資料庫直接查詢驗證，該 User ID 在所有關聯資料表中不得再出現任何紀錄（孤兒資料 = 0）。
