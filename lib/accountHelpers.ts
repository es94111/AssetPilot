...
export function getExchangeRateToTwd(
  userId: string,
  ...
  if (c === "TWD") return 1;
  const row = queryOne(
    "SELECT rate_to_twd FROM exchange_rates WHERE user_id = ? AND currency = ?",
    [userId, c],
  );
...
export function getExchangeRateToTwdAsDecimal(
  userId: string,
  currencyCode: string | null | undefined,
): string {
  const c = normalizeCurrency(currencyCode);
  ...
  return new Decimal(defaultRate)
...
export function formatAccount(
  ...
  return {
    ...
    balance:
      ...
      calcBalance(
        a.id as string,
        a.initial_balance as number,
        a.user_id as string,
        normalizeCurrency(a.currency as string),
      ),
...
export function convertToTwd(
  ...
  if (!Number.isFinite(original) || !(original > 0))
    throw new Error("金額必須為有限且大於 0 的數值");
  ...
  if (currency === "TWD") {
    ...
    if (!inputValue.isFinite() || inputValue.lessThanOrEqualTo(0)) {
      throw new Error("匯率必須為有限且大於 0 的數值");
    }
  ...
  if (!fxRateDecimal.isFinite() || fxRateDecimal.lessThanOrEqualTo(0)) {
    throw new Error("匯率必須為有限且大於 0 的數值");
  }
  const twdDecimal = new Decimal(original)
    .times(fxRateDecimal)
    .toDecimalPlaces(2, Decimal.ROUND_HALF_UP);
  if (!twdDecimal.isFinite()) throw new Error("換算後金額超出可接受範圍");
  const twdAmount = twdDecimal.toNumber();
...
export function resolveOverseasFee(opts: {
  ...
  if (!acc) return 0;
  const isCredit =
    acc.category === "credit_card" || acc.account_type === "信用卡";
...
export function creditCardStatementCycle(
  ...
  const daysInMonth = (y: number, mo: number) =>
    new Date(Date.UTC(y, mo, 0)).getUTCDate();
  const clampedClosing = (y: number, mo: number) =>
    Math.min(day, daysInMonth(y, mo));
  ...
  const startDate = new Date(
    Date.UTC(prevY, prevMo - 1, clampedClosing(prevY, prevMo) + 1),
  );
