...
- [ ] T002 [P] 建立目錄與檔案骨架 `lib/moneyDecimal.js`、`lib/taipeiTime.js`、`lib/exchangeRateCache.js`，每檔僅放一行 `module.exports = {};` 與檔頭一行 zh-TW 註解說明用途；`server.js` 暫不 require，避免破壞既有啟動
...
- [ ] T010 於 `server.js` `initDatabase()` 段（既有 `server.js:592` `accounts` 與 `server.js:602` `transactions` `CREATE TABLE`）下方緊接「002 schema migration」註解區塊；於該區塊**啟動前**執行 `fs.copyFileSync('./database.db', './database.db.bak.' + Date.now())`（僅當 `database.db` 存在時），並 `console.log('[migration 002] backup → ', backupPath)`（[plan.md](./plan.md) CT-1 / [data-model.md](./data-model.md) §3.1）
...
- [ ] T016 [P] 完成 `lib/moneyDecimal.js`（[research.md](./research.md) §2.3）：實作並 export `SMALLEST_UNIT_BY_CURRENCY = { TWD: 1, USD: 100, EUR: 100, GBP: 100, CNY: 100, SGD: 100, HKD: 100, JPY: 1, KRW: 1, BHD: 1000, KWD: 1000, OMR: 1000, JOD: 1000 }`、`getSmallestUnit(currency)`（含 `Intl.NumberFormat` fallback）、`computeTwdAmount(amountInt, fxRateStr, fxFeeInt)`（內部以 `new Decimal(amountInt).times(fxRateStr).plus(fxFeeInt).toDecimalPlaces(0, Decimal.ROUND_HALF_UP).toNumber()` 回傳整數）、`formatForDisplay(amountInt, currency)`（依 minor unit 格式化字串）；檔頭 `const Decimal = require('decimal.js');`
...
- [ ] T020 於 `server.js` 既有 middleware 區段（鄰近 `authMiddleware`）新增 `requireOwnedAccount(req, res, next)`（[research.md](./research.md) §8）：從 `req.params.accountId` 取 ID；`SELECT id, user_id, currency, category, overseas_fee_rate, updated_at FROM accounts WHERE id = ?`；若無 row 或 `user_id !== req.userId` 一律 `res.status(404).json({ error: 'NotFound' })`；通過後 `req.account = row` 暴露給 handler；同檔再實作 `requireOwnedTransaction(req, res, next)`（讀 `req.params.txId`，同樣回 404，附帶檢查 `req.tx.account_id` 對應帳戶亦屬同 user）
- [ ] T021 [P] 於 `server.js` 工具函式區新增 `assertOptimisticLock(table, id, expectedUpdatedAt)`（[research.md](./research.md) §5）：`SELECT updated_at FROM <table> WHERE id = ? LIMIT 1`；不存在則 throw `{ status: 404, error: 'NotFound' }`；不符則 throw `{ status: 409, error: 'OptimisticLockConflict', serverUpdatedAt: row.updated_at, message: '此筆已被其他裝置修改，請重新整理後再操作' }`；通過則 return；caller 以 `try { assertOptimisticLock(...) } catch (e) { return res.status(e.status).json(e); }` 模式使用
- [ ] T022 [P] 於 `server.js` 既有 `createDefaultsForUser(userId)`（`server.js:873`）末尾新增三段 INSERT（FR-002 / FR-020a；[data-model.md](./data-model.md) §2.1, §2.4）：(a) 預設「現金」帳戶 — `INSERT INTO accounts (id, user_id, name, category, initial_balance, currency, icon, exclude_from_total, linked_bank_id, overseas_fee_rate, created_at, updated_at) VALUES (?, ?, '現金', 'cash', 0, 'TWD', 'fa-wallet', 0, NULL, NULL, ?, ?)`；(b) 預設 `user_settings` — `INSERT INTO user_settings (user_id, pinned_currencies, updated_at) VALUES (?, '["TWD"]', ?)`；(c) `saveDB()` 已由外層呼叫，此處不重複；測試方式：刪除測試 user 後以 `/api/auth/register` 重新註冊，DB 應同步出現上述兩列
...
- [ ] T037 [US1] 於 `server.js` 新增 `PATCH /api/transactions/:txId`（FR-014、FR-014a）：套 `authMiddleware` + `requireOwnedTransaction`；body 取 `expected_updated_at` → `assertOptimisticLock('transactions', id, ...)`；可改欄位限 `amount`、`currency`、`fxRate`、`fxFee`、`date`、`categoryId`、`accountId`、`note`、`excludeFromStats`（**禁** type 變更、禁 transfer_* 之 PATCH——若 `req.tx.type` 為 transfer_*，回 422 `{ error: 'TransferImmutable', message: '轉帳交易僅能整對刪除，無法逐筆編輯欄位（請改用刪除後重建）' }`）；改 currency／amount/fxRate/fxFee 則重算 `twd_amount`；UPDATE 後 `updated_at = Date.now()` 並 `saveDB()`；回 200 + 更新後 Transaction
- [ ] T038 [US1] 於 `server.js` 新增 `DELETE /api/transactions/:txId`（FR-014、FR-014a；FR-015 連動由 US3 處理但 base 邏輯於此實作）：套 `authMiddleware` + `requireOwnedTransaction`；body 取 `expected_updated_at` → `assertOptimisticLock`；包 `BEGIN`：若 `req.tx.linked_id !== ''` 則同 transaction 內 `DELETE FROM transactions WHERE (id = ? OR id = ?) AND user_id = ?`（`req.tx.id` + `req.tx.linked_id`），否則 `DELETE FROM transactions WHERE id = ? AND user_id = ?`；`COMMIT` + `saveDB()`；回 204
...
- [ ] T072 [US3] 於 `server.js` 月支出／分類統計查詢 SQL（既有 `server.js:3134` 與相關 dashboard SQL）追加 `AND type IN ('income','expense')` 過濾（FR-016）；確認所有「月支出」、「分類統計」、「預算進度」對應的 SQL 都加入此過濾，不計入 transfer_*
...
- [ ] T112 [US5] [P] 於 `server.js` 新增 `PUT /api/user/settings/pinned-currencies`：套 `authMiddleware`；body `{ pinnedCurrencies: string[] minItems 1 maxItems 50, expected_updated_at }`；驗證每筆 `^[A-Z]{3}$`、必含 `'TWD'`（避免使用者誤刪）；`assertOptimisticLock('user_settings', userId, expected_updated_at)`（user_settings PK 即 user_id，需修改 helper 接受 PK 欄位名稱或寫一段 inline 比對）；UPDATE 後 `updated_at = Date.now()`；回 200 + 新狀態
- [ ] T113 [US5] 於 T035（`POST /api/transactions`）邏輯加入信用卡海外手續費預設邏輯（FR-021）：`req.account.category === 'credit_card' && currency !== 'TWD'` 時，若 body 未提供 `fxFee` 則計算 `defaultFeeRate = req.account.overseas_fee_rate ?? 150`（千分點，未設 fallback 1.5%）→ `fxFee = moneyDecimal.computeTwdAmount(amount, fxRate, 0) * defaultFeeRate / 1000` 取整；若 body 提供 `fxFee` 則直接使用（覆寫不回寫帳戶 — FR-021）
...
- [ ] T122 [US5] 於 `app.js` 交易 Modal 切幣別後自動匯率填入（FR-020、SC-003）：使用者切幣別至非 TWD 時呼叫 `GET /api/exchange-rates/:currency`；若 200 則填入 `fxRate` 欄位（read-only，但提供「鎖頭」icon 點擊解鎖以手動調整）；同時 TWD 等值顯示欄位呼叫前端版的 `computeTwdAmount`（共用 `lib/moneyDecimal.js` 邏輯，前端可動態 `<script>` 載入或於 `app.js` 重寫一份簡化版本）即時更新；若 503 顯示「匯率暫不可用，請手動輸入」、`fxRate` 欄位轉為一般輸入
- [ ] T123 [US5] 於 `app.js` 信用卡海外手續費 UI（FR-021）：`accounts[selectedId].category === 'credit_card' && currency !== 'TWD'` 時，Modal 顯示「海外刷卡手續費」欄位，預設值取該卡 `overseasFeeRate / 1000` × 計算出的 TWD 等值（前端先以該卡 overseasFeeRate fallback 1.5% 計算，後端提交時若 fxFee 缺失則由 T113 同樣公式算出，兩端一致）；標籤顯示「{rate}%（可調整）」；勾選「不收手續費」可將 fxFee 強制設為 0
...
- **Phase 9 Polish**：需所有預定 stories 完成後執行；T140~T143 可 [P]。
...
- **US5（P2）**：依 US1 完成（單筆交易 + Modal 已存在）；T113 補強既有 `POST /api/transactions` 行為；可與 US3、US4 並行。
...
- **Polish**：T140 / T141 / T142 / T143 / T149 [P] 可並行（各動不同檔案）。
