...
//
// App 在登入／註冊時向 Google Play 取得 integrity token，後端在此解碼出
...
import { evaluateVerdict, getPackageName, type IntegrityVerdict } from './playIntegrityVerdict';
...
function getServiceAccount(): ServiceAccount | null {
  ...
  try {
    ...
    if (!parsed.client_email || !parsed.private_key) {
      logger.warn('[playIntegrity] service account JSON 缺少 client_email / private_key');
      serviceAccountCache = null;
      return null;
    }
    // 由 .env 讀入時 \n 常被轉成字面字串，需還原為實際換行。
    ...
    logger.warn({ err: String(e) }, '[playIntegrity] 無法解析 service account JSON');
    serviceAccountCache = null;
    return null;
  }
}
...
export function isPlayIntegrityEnforced(): boolean {
  return String(process.env.PLAY_INTEGRITY_ENFORCE || '').trim().toLowerCase() === 'true';
}
...
async function getAccessToken(sa: ServiceAccount): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logger.warn({ status: res.status, body: text.slice(0, 300) }, '[playIntegrity] 取得 access token 失敗');
      return null;
    }
...
async function decodeIntegrityToken(integrityToken: string): Promise<IntegrityVerdict | null> {
  ...
  try {
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => '');
      logger.warn({ status: res.status, body: text.slice(0, 300) }, '[playIntegrity] decodeIntegrityToken 失敗');
      return null;
    }
    const data = await res.json();
    return (data?.tokenPayloadExternal || null) as IntegrityVerdict | null;
  } catch (e) {
    logger.warn({ err: String(e) }, '[playIntegrity] decodeIntegrityToken 例外');
    return null;
  } finally {
    clearTimeout(timeoutId);
  }
...
export async function verifyIntegrity(params: {
  ...
  try {
    ...
    logger.warn({ err: String(e), context }, '[playIntegrity] verifyIntegrity 例外');
  }
